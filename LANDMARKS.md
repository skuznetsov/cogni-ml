...
**decision:** Keep `lowrank-ffn-pca-updown-R` as a research branch, but deprioritize simple runtime controllers (`fallback`, `full-accept streak`) as a production speed path for the current 27B band. Do not promote pca-updown default. Next pca-updown work needs a predictive risk signal before enabling, different calibration, or a narrower layer/prompt policy; otherwise move to other exact levers.
**adversary:** Warmup is exact and guards actual use, but it still lacks a pre-enable quality predictor. Because the overlap scheduler submits the next draft block before verifying the current one, enabling pca-updown can lag by one block and disabling still pays for the first bad pca chunk. The refutation is suite-limited, but strong enough to avoid more broad always-on/warmup retesting without a new hypothesis.
