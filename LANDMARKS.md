...
- claim: "`--simulate-self-spec-gpu-pipeline-route-features` adds prompt-level route-risk instrumentation for same-weight self-draft route search. It prints held-out PCA residual count/mean/p50/p90/p99/max and threshold pass rates for the selected prompt and layer set, and suite hybrid sweeps enable it automatically. The intended use is to correlate cheap residual geometry with route stability before selecting no-FFN/updown maps; it is not yet a route-selection policy."
  source: build `CRYSTAL_CACHE_DIR=/tmp/cogni_ml_route_features crystal build bin/qwen35_deltanet_fixed_basis_probe.cr -o /tmp/qwen35_route_features_probe --link-flags="/tmp/cogni_ml_bridge_pipeline.o -framework Metal -framework Foundation -framework MetalPerformanceShaders -lc++"`; smokes `/tmp/qwen35_route_features_smoke.log` and `/tmp/qwen35_route_features_suite_smoke.log`; top2 spec `crystal spec spec/qwen35_decode_top2_spec.cr --link-flags="/tmp/cogni_ml_bridge_pipeline.o -framework Metal -framework Foundation -framework MetalPerformanceShaders -lc++"` (`1 example, 0 failures`), on 2026-04-29
  verified_at: 2026-04-29
