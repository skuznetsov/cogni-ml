...
**adversary:** Fallback-on-reject is reactive, not predictive: it still pays for the first bad pca-updown chunk and any already-submitted next draft block. The JSON recovery may depend on rejection placement. Exact verification protects emitted parity, but wall wins can vanish if rejections occur late or host load shifts verifier timing. This is a practical guard, not proof that global pca-updown is robust.
