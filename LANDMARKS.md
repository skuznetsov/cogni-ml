...
- claim: "The first 27B router-data mini-gate favors the 2B draft over the 0.8B draft on a narrow 4-prompt, `tokens=4` slice. With target `Qwen3.6-27B-Q4_K_M`, policies `default,ngram`, and prompts `France`, forced repetition, `def fibonacci`, and `The quick brown fox`, 2B averaged `88.87ms/tok` default and `86.77ms/tok` n-gram at `87.5%` acceptance, while 0.8B averaged `100.14ms/tok` default and `98.73ms/tok` n-gram at `81.25%` acceptance. The cycle summaries show why this needs a router: 2B neural cycles produced positive aggregate expected gain (`374.3ms`) with fewer rejects (`2`) than 0.8B (`10.0ms`, `4` rejects), while n-gram's real win came from the forced repeated suffix prompt."
  source: `/tmp/qwen35_speculative_sweep_summary --runner /tmp/qwen35_spec_accept_dump_cycles3 --tokens 4 --reps 1 --policies default,ngram --only-prompts "The capital of France is|alpha beta gamma delta alpha beta gamma delta|def fibonacci(n):|The quick brown fox" --extra-arg --target --extra-arg ...Qwen3.6-27B-Q4_K_M.gguf --extra-arg --draft --extra-arg ...Qwen3.5-0.8B-Q8_0.gguf --extra-arg --ngram-min --extra-arg 2 --extra-arg --ngram-max --extra-arg 8 --dump-cycles-dir /tmp/qwen35_router_dataset_27b_08b_4p` and the same command with `...Qwen3.5-2B-Q4_K_M.gguf`, on 2026-04-29
  verified_at: 2026-04-29
  decay_trigger: prompt suite, generated length, draft model files, n-gram thresholds, gamma/adaptive policy, host load, or cycle expected-gain labeling
**decision:** Keep `lowrank-ffn-pca-updown-R` as a research branch, but deprioritize simple runtime controllers (`fallback`, `full-accept streak`) as a production speed path for the current 27B band. Do not promote pca-updown default. The active exact-speed pivot is now data-driven speculative routing: collect cycle-level neural/ngram/target-only examples, then train or hand-fit a fail-closed router.
**adversary:** JSONL dumping is instrumentation, not a direct speedup. The `expected_gain_ms` label is relative to the post-run average plain target ms/token, so it is a useful noisy training signal rather than a formal per-cycle oracle. Raw token ids are opt-in because even token ids can leak private prompts. The next gate should include multi-prompt dumps and check that dump overhead is not included in benchmark-critical runs unless explicitly requested.
