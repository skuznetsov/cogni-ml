...
**decision:** Keep `lowrank-ffn-pca-updown-R` as an active exact-verifier research speed branch for Qwen3.6-27B narrow band `24,25,26,28,29,30`, with `rank16`, `draft_split=1`, and fallback-on-first-reject as a guard. Do not promote it to production decode yet: current self-spec remains slower than native plain exact, and the suite falsifier shows prompt-dependent wall regressions. The next gate is a predictive/prompt-aware or schedule-aware controller, not wider always-on pca-updown.
**adversary:** Fallback-on-reject is reactive, not predictive: it still pays for the first bad pca-updown chunk and any already-submitted next draft block. The JSON recovery may depend on rejection placement. Exact verification protects emitted parity, but wall wins can vanish if rejections occur late or verifier/replay dominates. The suite harness reduces measurement overhead and false adapter reuse, but it is not a speedup by itself and still rebuilds prompt-local K bases per eval prompt.
