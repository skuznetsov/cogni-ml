...
**adversary:** This is unlikely to help decode (`T=1`) and may not help short pp64 prefill. Dense summaries are now kept only as a long-prefill GPU baseline; compact summaries remain the pp256+ path. The current Metal evidence covers compact apply plus compact-`A`/`B` construction, but still excludes prefix-scan composition, replay, and prefill integration. Do not replace the existing rowwise kernel without a full `build + scan/apply + replay` A/B against `delta_net_chunk_128_rowwise`.
