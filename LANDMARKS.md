...
**adversary:** This is unlikely to help decode (`T=1`) and may not help short pp64 prefill. Dense summaries are now kept only as a long-prefill GPU baseline; compact summaries remain the pp256+ path. The current Metal evidence covers compact apply and compact-`A` construction, but still excludes compact-`B` right-factor construction, prefix-scan composition, replay, and prefill integration. Do not replace the existing rowwise kernel without a full `build + scan/apply + replay` A/B against `delta_net_chunk_128_rowwise`.
