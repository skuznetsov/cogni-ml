...
- claim: "`--bootstrap-streak` adds an exact control knob for prompt/risk-aware speculative growth. With `--bootstrap-gamma 32 --bootstrap-streak 2`, the harness waits for two full-accept chunks at the initial gamma before jumping to gamma32, avoiding the one-shot `bootstrap32` collapse on `def fibonacci(n):`. A 3-prompt gate (`France`, `def fibonacci(n):`, `quick brown`) showed `bootstrap32_s2` winning `3/3` vs default (`19.02ms/tok` avg vs `19.64`, paired ratio `0.962x`) while keeping exact parity; a broader 8-prompt gate was mixed/noisy (`6/8` wins, paired ratio `1.007x`), so the policy is a promising opt-in, not a production default yet."
  source: `/tmp/qwen35_speculative_sweep_bootstrap_streak --policies default,bootstrap32,bootstrap32_s2 --only-prompts "The capital of France is|def fibonacci(n):|The quick brown fox"` and `/tmp/qwen35_speculative_sweep_bootstrap_streak --policies default,bootstrap32_s2` over an 8-prompt mix, Qwen3.5 9B target plus Qwen3.5 0.8B Q8_0 draft, on 2026-04-28
  verified_at: 2026-04-28
...
**decision:** Keep `lowrank-ffn-pca-down-R` and `lowrank-ffn-pca-updown-R` as safe exact-verifier research gates. `WdB` factorization is correct and compact; `pca-updown` is the more important speed candidate because it can remove dense `gate/up` as well as `ffn_down` in selected draft layers. Do not promote `pca-updown` as a production sidecar yet. The integrated layer-kernel is useful for known-span/layer-major chunks, but the single-token self-draft route is not a speed path. The next speed target must keep multiple draft layers/tokens resident through logits/top1. The GPU embedding primitive removes the first concrete blocker for `top1_id_buf -> next hidden_buf`; the queued chain proves the handoff semantics; and the real fixed-gamma scheduler now validates the intended `draft_chain[k+1] || verify(draft_ids[k])` control flow. Current low-rank self-spec is not yet a plain exact decode win; it is a scheduling substrate plus research probe. Next gate: make draft blocks materially cheaper than exact decode (pca-updown, fewer layers, CPU/off-GPU draft, or GPU accept-only verifier) and compare against both native plain exact and llama.cpp, not just serial draft+verify. Do not chase wave chunk-size tuning or no-backup rollback as the main path.
**adversary:** PCA component sign alone should not matter: if a basis vector `b` is replaced by `-b` and `WdB` is recomputed consistently, `(b^T a) * (W_down b)` is invariant. A sign bug could matter only if `B` and `WdB` are serialized/reloaded with inconsistent signs, which this in-memory probe does not do. The `gen=16` gate shows a real rejection tax outside one 27B band; any production use needs either stronger calibration/training, dynamic band/rank gating, or an acceptance-risk controller. The integrated kernel timing is a known-span layer-chunk gate, not full autoregressive wall speed; the routed wall gate confirms that consuming it at per-layer single-token granularity erases the local kernel gain.
