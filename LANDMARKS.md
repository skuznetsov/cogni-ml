...
**decision:** Keep `lowrank-ffn-pca-updown-R` as an active exact-verifier research speed branch for Qwen3.6-27B narrow band `24,25,26,28,29,30`, with `rank16`, `draft_split=1`, first-reject fallback, and warmup-after-full-accepts as the next policy candidate. Do not promote it to production decode yet: current self-spec remains slower than native plain exact, and the suite falsifier shows prompt-dependent wall regressions. Next gate: run the same 27B suite with warmup values `1/2` and compare against baseline/fallback.
**adversary:** Warmup is conservative and exact, but it can also erase useful pca-updown work on short generations. Because the overlap scheduler submits the next draft block before verifying the current one, enabling pca-updown can lag by one already-submitted block. Fallback-on-reject remains reactive, so prompt classes with late rejections can still waste work. The policy needs 27B suite evidence before it becomes more than a guarded research option.
