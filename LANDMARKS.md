...
**decision:** Keep `lowrank-ffn-pca-updown-R` as an active exact-verifier research speed branch for Qwen3.6-27B narrow band `24,25,26,28,29,30`, with `rank16` as the first cost candidate and `draft_split=1` as the scheduler default. Do not promote it to production decode yet: current self-spec remains slower than native plain exact, and external calibration shows prompt-dependent acceptance regressions. Use in-process A/B (`draft-updowns=0,16,32`) for future claims because separate-run `plain_exact_ms` is noisy. The next gate should be either a prompt-risk controller that falls back to baseline lowrank when pca-updown hurts acceptance, or a better/global calibration method before another large scheduler rewrite.
**adversary:** The positive prompt-local `gen=16` result does not generalize cleanly under external calibration. Exact verification protects emitted parity, but extra rejections can erase or reverse the local kernel gain. Rank32 is not a universal fix for the JSON/db-shards failure. The current evidence supports `pca-updown` as a gated draft-cost lever, not as a default sidecar.
