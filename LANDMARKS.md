...
**decision:** Keep `lowrank-ffn-pca-updown-R` as an active exact-verifier research speed branch for Qwen3.6-27B narrow band `24,25,26,28,29,30`, with `rank16` as the first cost/quality candidate and `draft_split=1` as the current scheduler default. Do not promote it to production decode yet: current self-spec remains slower than native plain exact on the measured prompt, and the acceptance gain may be prompt/calibration-specific. Use in-process A/B (`draft-updowns=0,16,32`) for future claims because separate-run `plain_exact_ms` is too noisy. The next gate is a 27B prompt suite at `gen=16/32`, external/global FFN calibration, and then scheduler work to reduce `draft_wait_ms` or verifier cost.
**adversary:** The positive `gen=16` result is not enough to generalize: baseline lowrank had a static top1 mismatch on the held-out generated path, and prompt-local `pca-updown` may be acting as a lucky calibration correction rather than a robust lower-cost FFN surrogate. Exact verification protects emitted parity, but rejection economics can erase the win on broader prompts. The rows-kernel lane split preserves the same mathematical operation up to FP reduction order; any claim beyond the tested smokes needs fresh parity/acceptance gates after Metal compiler or kernel-layout changes.
