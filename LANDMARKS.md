...
- claim: "Resident branch-state copies are viable for exact tree speculative verification. `Qwen35CPU.copy_state_metal_used!` / `encode_state_metal_used_copy!` now copy recurrent DeltaNet state in full while copying full-attention KV only up to each layer's live `position`. The microbench `bin/qwen35_branch_state_copy_micro.cr` measured blit-used copies much cheaper than old full-capacity `State#copy_from!`: at `max_seq=2048,pos64`, two branch copies cost `0.937ms` on 9B and `2.350ms` on 27B, versus old full-state memcpy `7.194ms` and `16.261ms`; eight 27B branches at `pos64` cost `8.910ms` versus `67.089ms`. At `pos2048`, eight 27B branches copy `3.245GiB` in `20.411ms` via blit-used, still about `3.3x` faster than memcpy. This makes DeltaNet/KV branch storage/copy a manageable cost, not the primary blocker for `k=2`/small-K tree verification."
  source: `CRYSTAL_CACHE_DIR=/tmp/cogni_ml_branch_copy_build crystal build bin/qwen35_branch_state_copy_micro.cr -o /tmp/qwen35_branch_state_copy_micro --link-flags="$(pwd)/build/bridge.o -framework Metal -framework Foundation -lc++"`, `/tmp/qwen35_branch_state_copy_micro --models=default --max-seq=2048 --positions=64,512,2048 --branches=1,2,4,8 --reps=5 --warmup=2`, log `/tmp/qwen35_branch_state_copy_9b_27b_pos64_512_2048.log`, and `CRYSTAL_CACHE_DIR=/tmp/cogni_ml_state_copy_spec crystal spec spec/qwen35_state_copy_spec.cr --link-flags="$(pwd)/build/bridge.o -framework Metal -framework Foundation -lc++"` (`1 examples, 0 failures`), on 2026-04-29
  verified_at: 2026-04-29
  decay_trigger: state buffer layout, KV position semantics, Metal blit path, branch-state pool implementation, prompt length, model hparams, or M2 Max memory-load conditions
  adversary_update: This proves state copy is cheap enough to try tree verification, not that tree verification is profitable. The next falsifier must include exact branch verifier cost, rank-ordered early exit, and rescue frequency on real refute prompts.
