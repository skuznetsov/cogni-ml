...
- [ ] **4.9** True layerwise/microbatch prefill for known prompt tokens
  ...
  - [x] Promote exact full-row batched Q6 lm-head verifier for `prefill_tokens_top1s` rows above the GEMM threshold (`QWEN35_HEAD_FULL_ROWS_OFF=1` disables). This matches llama.cpp's key trick of evaluating verifier logits as a batch instead of per-row top1, then reduces batched F16 logits on GPU: paired A/B wins on verifier chunks b16/b32/b64, with b64 p50 improving `557.39 -> 165.69 ms` under the same host load, while focused Qwen specs stay green; high-accept staged smoke reaches `12.26 ms/tok`
