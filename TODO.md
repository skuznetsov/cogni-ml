...
- [ ] **4.9** True layerwise/microbatch prefill for known prompt tokens
  ...
  - [x] Add an env-tunable Q4 pair-H16 prefill threshold (`QWEN35_Q4K_PAIR_H16_MIN_BATCH`, default `256`) after testing the missing `128` point. Lowering the threshold from `256` to `128` activates shared input conversion at pp128 but is neutral (`~268.0 ms`, `3/6` wins), while pp64 remains off/noisy and pp256 preserves the known positive signal; keep default `256`.
...
- [ ] **R.5** Exact DeltaNet associative block-scan prototype for long prefill.
  ...
  - [ ] Build default-off Metal microbench for fully compact block summaries at synthetic Qwen shape (`s=128`, block `8/16/32`) before integrating into prefill.
