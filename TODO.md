...
- [ ] **4.9** True layerwise/microbatch prefill for known prompt tokens
  ...
  - [x] Skip draft-state advance before imminent target-only fallback after early rejection (`QWEN35_SPEC_SKIP_DRAFT_BEFORE_FALLBACK_OFF=1` disables): on `def fibonacci(n):` the exact harness skips one unused draft step (`draft_skip=1`) and improves paired smokes from `20.85/21.54/21.60` to `20.78/20.75/21.40 ms/tok`; high-acceptance prompt has `draft_skip=0` and stays unchanged within noise
