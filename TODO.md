...
- [ ] **4.9** True layerwise/microbatch prefill for known prompt tokens
  ...
  - [x] Add exact first-candidate early reject for chunk verifiers: if `candidate[0] != target_next`, skip the full target chunk, advance only the corrected target token, and resync draft. On `def fibonacci(n):`, gamma=4/tokens=64 improves from `32.77/32.89 ms/tok` with `QWEN35_SPEC_EARLY_REJECT_OFF=1` to `30.02/30.85 ms/tok` default
