...
kernel void simd_mv_q4k_f32_x16(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const float*   x       [[buffer(1)]],
    device       float*   output  [[buffer(2)]],
    constant     uint&    in_dim  [[buffer(3)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    ...
    const short tx = tiisg & 15;  // reduction lane within the row
    const short ty = tiisg >> 4;  // row lane within the simdgroup: 0..1
    const short ix = tx / 8;      // 0..1 block stride within this row group
    const short it = tx % 8;
    const short iq = it / 4;
    const short ir = it % 4;

    const uint nb = in_dim / QK_K;
    const uint row = (tgpig.x * MV_NSG + sgitg) * 2 + ty;
    const uint n = tgpig.y;
    if (row >= out_dim || n >= batch) return;
    ...
    for (uint ib = ix; ib < nb; ib += 2) {
        ...
        device const block_q4_K * blk =
            (device const block_q4_K *)(w_raw + row * row_bytes) + ib;
...
constant int MM48_NR0 = 64;
constant int MM48_NR1 = 48;
constant int MM48_SA_SIZE   = 4096;
constant int MM48_SB_SIZE   = 3072;
constant int MM48_TILE_SIZE = MM48_SA_SIZE + MM48_SB_SIZE;

// Wider batch tile variant for pre-converted H16 inputs. It keeps the same
// 64 output rows but uses 8 simdgroups to cover 64 batch rows, so one
// dequantized weight tile feeds twice as many prompt rows.
constant int MM64_NR0 = 64;
constant int MM64_NR1 = 64;
constant int MM64_SA_SIZE   = 4096;
constant int MM64_SB_SIZE   = 4096;
constant int MM64_TILE_SIZE = MM64_SA_SIZE + MM64_SB_SIZE;

// B128 experiment which retains the proven 8-simdgroup launch shape. Each
// simdgroup consumes one weight tile and two independent 16-row input bands.
constant int MM128_SG8_NR0 = 64;
constant int MM128_SG8_NR1 = 128;
constant int MM128_SG8_SA_SIZE   = 4096;
constant int MM128_SG8_SB_SIZE   = 8192;
constant int MM128_SG8_TILE_SIZE = MM128_SG8_SA_SIZE + MM128_SG8_SB_SIZE;

constant int MM80_NR0 = 64;
constant int MM80_NR1 = 80;
constant int MM80_SA_SIZE   = 4096;
constant int MM80_SB_SIZE   = 5120;
constant int MM80_TILE_SIZE = MM80_SA_SIZE + MM80_SB_SIZE;

constant int MM96_NR0 = 64;
constant int MM96_NR1 = 96;
constant int MM96_SA_SIZE   = 4096;
constant int MM96_SB_SIZE   = 6144;
constant int MM96_TILE_SIZE = MM96_SA_SIZE + MM96_SB_SIZE;

constant int MM112_NR0 = 64;
constant int MM112_NR1 = 112;
constant int MM112_SA_SIZE   = 4096;
constant int MM112_SB_SIZE   = 7168;
constant int MM112_TILE_SIZE = MM112_SA_SIZE + MM112_SB_SIZE;
...
kernel void simd_mm_q4k_h16_b48(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    device       float*   output  [[buffer(2)]],
    constant     uint&    in_dim  [[buffer(3)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    threadgroup half * sa_buf[2] = {
        (threadgroup half *)(shmem),
        (threadgroup half *)(shmem + MM48_TILE_SIZE)
    };
    threadgroup half * sb_buf[2] = {
        (threadgroup half *)(shmem + MM48_SA_SIZE),
        (threadgroup half *)(shmem + MM48_TILE_SIZE + MM48_SA_SIZE)
    };

    const int r0 = tgpig.y * MM48_NR0;
    const int r1 = tgpig.x * MM48_NR1;

    const short nr0 = min(MM48_NR0, (int)out_dim - r0);
    const short nr1 = min(MM48_NR1, (int)batch   - r1);
...

    {
        ...
        }
        {
            ...
            const short sy = (tiitg/MM_NL1)/8;
            const short ly = (tiitg/MM_NL1)%8;
            threadgroup half * dst = sb + 64*(6*sx + sy) + 8*ly;
            *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
        }
        il = (il + 2 < MM_NL) ? il + 2 : il % 2;
        xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
    ...
    for (uint iter = 0; iter < n_iter; iter++) {
        ...
        if (iter + 1 < n_iter) {
            ...
            }
            {
                ...
                const short sy = (tiitg/MM_NL1)/8;
                const short ly = (tiitg/MM_NL1)%8;
                threadgroup half * dst = sb_n + 64*(6*sx + sy) + 8*ly;
                *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
            }
            il = (il + 2 < MM_NL) ? il + 2 : il % 2;
            xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
    ...
    threadgroup float * temp = (threadgroup float *)shmem;
    {
        threadgroup float * sg_out = temp + 32*(sgitg & 1) + 16*(sgitg >> 1)*MM48_NR0;
        for (short i = 0; i < 8; i++) {
            simdgroup_store(mc[i], sg_out + 8*(i%4) + 8*MM48_NR0*(i/4), MM48_NR0, 0, false);
        }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    const int total_out = nr0 * nr1;
    for (int idx = (int)tiitg; idx < total_out; idx += 192) {
        const int i = idx % nr0;
        const int j = idx / nr0;
        output[(r1 + j) * out_dim + r0 + i] = temp[j * MM48_NR0 + i];
    }
}

kernel void simd_mm_q4k_h16_b64(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    device       float*   output  [[buffer(2)]],
    constant     uint&    in_dim  [[buffer(3)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    threadgroup half * sa_buf[2] = {
        (threadgroup half *)(shmem),
        (threadgroup half *)(shmem + MM64_TILE_SIZE)
    };
    threadgroup half * sb_buf[2] = {
        (threadgroup half *)(shmem + MM64_SA_SIZE),
        (threadgroup half *)(shmem + MM64_TILE_SIZE + MM64_SA_SIZE)
    };
...

    {
        ...
        }
        {
            ...
            const short sy = (tiitg/MM_NL1)/8;
            const short ly = (tiitg/MM_NL1)%8;
            threadgroup half * dst = sb + 64*(8*sx + sy) + 8*ly;
            *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
        }
        il = (il + 2 < MM_NL) ? il + 2 : il % 2;
        xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
    ...
    for (uint iter = 0; iter < n_iter; iter++) {
        ...
        if (iter + 1 < n_iter) {
            ...
            }
            {
                ...
                const short sy = (tiitg/MM_NL1)/8;
                const short ly = (tiitg/MM_NL1)%8;
                threadgroup half * dst = sb_n + 64*(8*sx + sy) + 8*ly;
                *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
            }
            il = (il + 2 < MM_NL) ? il + 2 : il % 2;
            xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
    ...
    threadgroup float * temp = (threadgroup float *)shmem;
    {
        threadgroup float * sg_out = temp + 32*(sgitg & 1) + 16*(sgitg >> 1)*MM64_NR0;
        for (short i = 0; i < 8; i++) {
            simdgroup_store(mc[i], sg_out + 8*(i%4) + 8*MM64_NR0*(i/4), MM64_NR0, 0, false);
        }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (int idx = (int)tiitg; idx < total_out; idx += 256) {
        const int i = idx % nr0;
        const int j = idx / nr0;
        output[(r1 + j) * out_dim + r0 + i] = temp[j * MM64_NR0 + i];
    }
}

// Exact-shape B128 variant: reuse each dequantized 64x32 weight tile across
// two 64-row input bands while preserving the B64 kernel's 8-simdgroup launch.
// The host admits only complete 64x128 tiles, so no cooperative tail store is
// needed and the 24-KiB threadgroup allocation is entirely double buffering.
kernel void simd_mm_q4k_h16_b128_sg8(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    device       float*   output  [[buffer(2)]],
    constant     uint&    in_dim  [[buffer(3)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    threadgroup half * sa_buf[2] = {
        (threadgroup half *)(shmem),
        (threadgroup half *)(shmem + MM128_SG8_TILE_SIZE)
    };
    threadgroup half * sb_buf[2] = {
        (threadgroup half *)(shmem + MM128_SG8_SA_SIZE),
        (threadgroup half *)(shmem + MM128_SG8_TILE_SIZE + MM128_SG8_SA_SIZE)
    };

    const int r0 = tgpig.y * MM128_SG8_NR0;
    const int r1 = tgpig.x * MM128_SG8_NR1;
    if (r0 + MM128_SG8_NR0 > (int)out_dim ||
        r1 + MM128_SG8_NR1 > (int)batch) return;
...

    {
        ...
        }
        {
            ...
            const short sy = (tiitg/MM_NL1)/8;
            const short ly = (tiitg/MM_NL1)%8;
            threadgroup half * dst0 = sb + 64*(16*sx + sy) + 8*ly;
            threadgroup half * dst1 = sb + 64*(16*sx + sy + 8) + 8*ly;
            *(threadgroup half2x4 *)dst0 = *(device const half2x4 *)y0;
            *(threadgroup half2x4 *)dst1 = *(device const half2x4 *)y1;
        }
        il = (il + 2 < MM_NL) ? il + 2 : il % 2;
        xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
        y0 += MM_NK;
    ...
    for (uint iter = 0; iter < n_iter; iter++) {
        ...
        threadgroup const half * lsma = sa + 4*64*(sgitg % 2);
        threadgroup const half * lsmb0 = sb + 2*64*(sgitg / 2);
        threadgroup const half * lsmb1 = sb + 2*64*((sgitg / 2) + 4);
        FOR_UNROLL for (short ik = 0; ik < MM_NK/8; ik++) {
            simdgroup_barrier(mem_flags::mem_none);
            FOR_UNROLL for (short i = 0; i < 4; i++) simdgroup_load(ma[i], lsma + 64*i, 8, 0, false);
            simdgroup_barrier(mem_flags::mem_none);
        ...
        if (iter + 1 < n_iter) {
            ...
            }
            {
                ...
                const short sy = (tiitg/MM_NL1)/8;
                const short ly = (tiitg/MM_NL1)%8;
                threadgroup half * dst0 = sb_n + 64*(16*sx + sy) + 8*ly;
                threadgroup half * dst1 = sb_n + 64*(16*sx + sy + 8) + 8*ly;
                *(threadgroup half2x4 *)dst0 = *(device const half2x4 *)y0;
                *(threadgroup half2x4 *)dst1 = *(device const half2x4 *)y1;
            }
            il = (il + 2 < MM_NL) ? il + 2 : il % 2;
    ...
    device float * C0 = output + (r0 + 32*(sgitg & 1)) +
                         (r1 + 16*(sgitg >> 1))*out_dim;
...
// B128 companion for the default H16 FFN corridor. It reuses the same weight
// tile for both 64-row input bands, then serializes the two elementwise
// epilogues through the reusable 64x64 F32 threadgroup tile.
kernel void simd_mm_q4k_h16_b128_sg8_swiglu_h16(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    ...
    threadgroup float * temp = (threadgroup float *)shmem;
    threadgroup float * sg_out = temp + 32*(sgitg & 1) +
                                 16*(sgitg >> 1)*MM128_SG8_NR0;
    FOR_UNROLL for (short i = 0; i < 8; i++) {
        simdgroup_store(mc0[i], sg_out + 8*(i%4) + 8*MM128_SG8_NR0*(i/4), MM128_SG8_NR0, 0, false);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    const int half_tile = MM128_SG8_NR0 * 64;
    for (int idx = (int)tiitg; idx < half_tile; idx += 256) {
        const int i = idx % MM128_SG8_NR0;
        const int j = idx / MM128_SG8_NR0;
        const uint out_idx = (r1 + j) * out_dim + r0 + i;
        const float g = gate[out_idx];
        const float sig = 1.0f / (1.0f + exp(-g));
        output[out_idx] = half((g * sig) * temp[idx]);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    FOR_UNROLL for (short i = 0; i < 8; i++) {
        simdgroup_store(mc1[i], sg_out + 8*(i%4) + 8*MM128_SG8_NR0*(i/4), MM128_SG8_NR0, 0, false);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (int idx = (int)tiitg; idx < half_tile; idx += 256) {
        const int i = idx % MM128_SG8_NR0;
        const int j = idx / MM128_SG8_NR0;
        const uint out_idx = (r1 + 64 + j) * out_dim + r0 + i;
        const float g = gate[out_idx];
        const float sig = 1.0f / (1.0f + exp(-g));
        output[out_idx] = half((g * sig) * temp[idx]);
    }
...
// Exact B64 up-projection + SwiGLU consumer fusion. This is only legal for
// full 64x64 tiles: it computes the Q4_K up projection, reads the already
// materialized gate projection, and writes silu(gate) * up to the output
// activation buffer. It intentionally stays opt-in because it trades the B64
// direct device store for a producer-consumer fusion.
kernel void simd_mm_q4k_h16_b64_swiglu(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    device const float*   gate    [[buffer(2)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    ...
    for (int idx = (int)tiitg; idx < total_out; idx += 256) {
        ...
        const float g = gate[out_idx];
        const float sig = 1.0f / (1.0f + exp(-g));
        output[out_idx] = (g * sig) * temp[j * MM64_NR0 + i];
    }
}
// Exact B64 up-projection + GELU consumer fusion. This is only legal for
// full 64x64 tiles: it computes the Q4_K up projection, reads the already
// materialized gate projection, and writes gelu(gate) * up to the output
// activation buffer. It intentionally stays opt-in because it trades the B64
// direct device store for a producer-consumer fusion.
kernel void simd_mm_q4k_h16_b64_gelu_mul(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    device const float*   gate    [[buffer(2)]],
    device       float*   output  [[buffer(3)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    ...
    for (int idx = (int)tiitg; idx < total_out; idx += 256) {
        ...
        const uint out_idx = (r1 + j) * out_dim + r0 + i;
        const float g = gate[out_idx];
        const float arg = clamp(0.7978845608028654f * g * (1.0f + 0.044715f * g * g), -10.0f, 10.0f);
        const float gelu = 0.5f * g * (1.0f + tanh(arg));
        output[out_idx] = gelu * temp[j * MM64_NR0 + i];
    }
}

// Exact B64 up-projection + SwiGLU fusion for downstream H16 FFN-down.
// This is exact relative to the existing swiglu_h16 route because the
// activation is rounded to half at the same producer-consumer boundary.
kernel void simd_mm_q4k_h16_b64_swiglu_h16(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    device const float*   gate    [[buffer(2)]],
    device       half*    output  [[buffer(3)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    ...
    for (int idx = (int)tiitg; idx < total_out; idx += 256) {
        ...
        const float g = gate[out_idx];
        const float sig = 1.0f / (1.0f + exp(-g));
        output[out_idx] = half((g * sig) * temp[j * MM64_NR0 + i]);
    }
}
// 96-row batch tile variant. Crystal only routes exact 96-token chunks here:
// broader use regressed because edge tiles waste too many simdgroups.

kernel void simd_mm_q4k_h16_b80(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    device       float*   output  [[buffer(2)]],
    constant     uint&    in_dim  [[buffer(3)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    threadgroup half * sa_buf[2] = {
        (threadgroup half *)(shmem),
        (threadgroup half *)(shmem + MM80_TILE_SIZE)
    };
    threadgroup half * sb_buf[2] = {
        (threadgroup half *)(shmem + MM80_SA_SIZE),
        (threadgroup half *)(shmem + MM80_TILE_SIZE + MM80_SA_SIZE)
    };

    const int r0 = tgpig.y * MM80_NR0;
    const int r1 = tgpig.x * MM80_NR1;

    const short nr0 = min(MM80_NR0, (int)out_dim - r0);
    const short nr1 = min(MM80_NR1, (int)batch   - r1);
...

    {
        ...
        if (tiitg < 128) {
            ...
            FOR_UNROLL for (short i = 0; i < 16; i++) {
                ...
                const short lx = (tidw/MM_NL0)%8;
                const short ly = i%8;
                *(sa + 64*(10*sx + sy) + 8*ly + lx) = temp_a[i/4][i%4];
            }
        }
        {
            const short sx = (tiitg % MM_NL1);
            const short sy = (tiitg/MM_NL1)/8;
            const short ly = (tiitg/MM_NL1)%8;
            threadgroup half * dst = sb + 64*(10*sx + sy) + 8*ly;
            *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
        }
        il = (il + 2 < MM_NL) ? il + 2 : il % 2;
        xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
        ...
        if (iter + 1 < n_iter) {
            ...
            if (tiitg < 128) {
                ...
                FOR_UNROLL for (short i = 0; i < 16; i++) {
                    ...
                    const short lx = (tidw/MM_NL0)%8;
                    const short ly = i%8;
                    *(sa_n + 64*(10*sx + sy) + 8*ly + lx) = temp_a[i/4][i%4];
                }
            }
            {
                const short sx = (tiitg % MM_NL1);
                const short sy = (tiitg/MM_NL1)/8;
                const short ly = (tiitg/MM_NL1)%8;
                threadgroup half * dst = sb_n + 64*(10*sx + sy) + 8*ly;
                *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
            }
            il = (il + 2 < MM_NL) ? il + 2 : il % 2;
            xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
    ...
    threadgroup float * temp = (threadgroup float *)shmem;
    {
        threadgroup float * sg_out = temp + 32*(sgitg & 1) + 16*(sgitg >> 1)*MM80_NR0;
        for (short i = 0; i < 8; i++) {
            simdgroup_store(mc[i], sg_out + 8*(i%4) + 8*MM80_NR0*(i/4), MM80_NR0, 0, false);
        }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (int idx = (int)tiitg; idx < total_out; idx += 256) {
        const int i = idx % nr0;
        const int j = idx / nr0;
        output[(r1 + j) * out_dim + r0 + i] = temp[j * MM80_NR0 + i];
    }
}
kernel void simd_mm_q4k_h16_b96(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    device       float*   output  [[buffer(2)]],
    constant     uint&    in_dim  [[buffer(3)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    threadgroup half * sa_buf[2] = {
        (threadgroup half *)(shmem),
        (threadgroup half *)(shmem + MM96_TILE_SIZE)
    };
    threadgroup half * sb_buf[2] = {
        (threadgroup half *)(shmem + MM96_SA_SIZE),
        (threadgroup half *)(shmem + MM96_TILE_SIZE + MM96_SA_SIZE)
    };

    const int r0 = tgpig.y * MM96_NR0;
    const int r1 = tgpig.x * MM96_NR1;

    const short nr0 = min(MM96_NR0, (int)out_dim - r0);
    const short nr1 = min(MM96_NR1, (int)batch   - r1);
...

    {
        ...
        if (tiitg < 128) {
            ...
            FOR_UNROLL for (short i = 0; i < 16; i++) {
                ...
                const short lx = (tidw/MM_NL0)%8;
                const short ly = i%8;
                *(sa + 64*(12*sx + sy) + 8*ly + lx) = temp_a[i/4][i%4];
            }
        }
        {
            const short sx = (tiitg % MM_NL1);
            const short sy = (tiitg/MM_NL1)/8;
            const short ly = (tiitg/MM_NL1)%8;
            threadgroup half * dst = sb + 64*(12*sx + sy) + 8*ly;
            *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
        }
        il = (il + 2 < MM_NL) ? il + 2 : il % 2;
        xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
        ...
        if (iter + 1 < n_iter) {
            ...
            if (tiitg < 128) {
                ...
                FOR_UNROLL for (short i = 0; i < 16; i++) {
                    ...
                    const short lx = (tidw/MM_NL0)%8;
                    const short ly = i%8;
                    *(sa_n + 64*(12*sx + sy) + 8*ly + lx) = temp_a[i/4][i%4];
                }
            }
            {
                const short sx = (tiitg % MM_NL1);
                const short sy = (tiitg/MM_NL1)/8;
                const short ly = (tiitg/MM_NL1)%8;
                threadgroup half * dst = sb_n + 64*(12*sx + sy) + 8*ly;
                *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
            }
            il = (il + 2 < MM_NL) ? il + 2 : il % 2;
            xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
    ...
    threadgroup float * temp = (threadgroup float *)shmem;
    {
        threadgroup float * sg_out = temp + 32*(sgitg & 1) + 16*(sgitg >> 1)*MM96_NR0;
        for (short i = 0; i < 8; i++) {
            simdgroup_store(mc[i], sg_out + 8*(i%4) + 8*MM96_NR0*(i/4), MM96_NR0, 0, false);
        }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (int idx = (int)tiitg; idx < total_out; idx += 256) {
        const int i = idx % nr0;
        const int j = idx / nr0;
        output[(r1 + j) * out_dim + r0 + i] = temp[j * MM96_NR0 + i];
    }
}

kernel void simd_mm_q4k_h16_b112(
    device const uint8_t* w_raw   [[buffer(0)]],
    device const half*    x       [[buffer(1)]],
    device       float*   output  [[buffer(2)]],
    constant     uint&    in_dim  [[buffer(3)]],
    ...
    ushort sgitg [[simdgroup_index_in_threadgroup]])
{
    threadgroup half * sa_buf[2] = {
        (threadgroup half *)(shmem),
        (threadgroup half *)(shmem + MM112_TILE_SIZE)
    };
    threadgroup half * sb_buf[2] = {
        (threadgroup half *)(shmem + MM112_SA_SIZE),
        (threadgroup half *)(shmem + MM112_TILE_SIZE + MM112_SA_SIZE)
    };

    const int r0 = tgpig.y * MM112_NR0;
    const int r1 = tgpig.x * MM112_NR1;

    const short nr0 = min(MM112_NR0, (int)out_dim - r0);
    const short nr1 = min(MM112_NR1, (int)batch   - r1);
...

    {
        ...
        if (tiitg < 128) {
            ...
            FOR_UNROLL for (short i = 0; i < 16; i++) {
                ...
                const short lx = (tidw/MM_NL0)%8;
                const short ly = i%8;
                *(sa + 64*(14*sx + sy) + 8*ly + lx) = temp_a[i/4][i%4];
            }
        }
        {
            const short sx = (tiitg % MM_NL1);
            const short sy = (tiitg/MM_NL1)/8;
            const short ly = (tiitg/MM_NL1)%8;
            threadgroup half * dst = sb + 64*(14*sx + sy) + 8*ly;
            *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
        }
        il = (il + 2 < MM_NL) ? il + 2 : il % 2;
        xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
        ...
        if (iter + 1 < n_iter) {
            ...
            if (tiitg < 128) {
                ...
                FOR_UNROLL for (short i = 0; i < 16; i++) {
                    ...
                    const short lx = (tidw/MM_NL0)%8;
                    const short ly = i%8;
                    *(sa_n + 64*(14*sx + sy) + 8*ly + lx) = temp_a[i/4][i%4];
                }
            }
            {
                const short sx = (tiitg % MM_NL1);
                const short sy = (tiitg/MM_NL1)/8;
                const short ly = (tiitg/MM_NL1)%8;
                threadgroup half * dst = sb_n + 64*(14*sx + sy) + 8*ly;
                *(threadgroup half2x4 *)dst = *(device const half2x4 *)y;
            }
            il = (il + 2 < MM_NL) ? il + 2 : il % 2;
            xw = (il < 2) ? xw + (2 + MM_NL - 1)/MM_NL : xw;
    ...
    if (nr0 == MM112_NR0 && nr1 == MM112_NR1) {
        device float * C = output + (r0 + 32*(sgitg & 1)) + (r1 + 16*(sgitg >> 1))*out_dim;
        for (short i = 0; i < 8; i++) {
            simdgroup_store(mc[i], C + 8*(i%4) + 8*out_dim*(i/4), out_dim, 0, false);
        }
    ...
    threadgroup float * temp = (threadgroup float *)shmem;
    {
        threadgroup float * sg_out = temp + 32*(sgitg & 1) + 16*(sgitg >> 1)*MM112_NR0;
        for (short i = 0; i < 8; i++) {
            simdgroup_store(mc[i], sg_out + 8*(i%4) + 8*MM112_NR0*(i/4), MM112_NR0, 0, false);
        }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    const int total_out = nr0 * nr1;
    for (int idx = (int)tiitg; idx < total_out; idx += 448) {
        const int i = idx % nr0;
        const int j = idx / nr0;
