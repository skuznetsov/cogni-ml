...
module ML::GGUF
  module DiffusionGemmaCPU
    ...
    struct PromptMaterializeTiming
      ...
      getter moe_grouped_down_ms : Float64
      getter moe_grouped_scatter_combine_norm_ms : Float64
      getter moe_grouped_active_experts : Int32
    ...
    struct FfnResidentExpertChunk
      getter expert : Int32
      getter chunk_index : Int32
      getter batch : Int32
      getter gather_map_buf : ML::MetalBuffer
      getter scatter_map_buf : ML::MetalBuffer
      getter input_buf : ML::MetalBuffer
      getter hidden_buf : ML::MetalBuffer
      getter down_buf : ML::MetalBuffer
      getter gate_up_buf : ML::MetalBuffer?
      getter gate_buf : ML::MetalBuffer?
      getter up_buf : ML::MetalBuffer?
    ...
    struct FfnResidentGraphCacheEntry
      getter graph : ML::Metal::ComputeGraph
      getter stats : ML::Metal::ComputeGraph::Stats

      def initialize(@graph, @stats)
      end
    end

    @@ffn_resident_graph_cache = {} of String => FfnResidentGraphCacheEntry
    ...
    struct PromptLayerCache
      ...
      getter materialize_moe_grouped_active_experts_by_layer : Array(Int32)
      ...
      def initialize(@final_rows,
                     ...
                     @materialize_moe_grouped_active_experts_by_layer = [] of Int32,
    ...
    def ffn_residual_rows_resident_graph(weights : DiffusionGemmaWeights,
      ...
      assignments_by_expert.keys.sort.each do |expert|
        ...
        while chunk_start < assignments.size
          ...
          gate_up_buf = nil.as(ML::MetalBuffer?)
          gate_buf = nil.as(ML::MetalBuffer?)
          up_buf = nil.as(ML::MetalBuffer?)
          ...
          if combined_gate_up_enabled
            ...
            owned_buffers << gate_up_buf.not_nil!
          else
            ...
            owned_buffers.concat([gate_buf.not_nil!, up_buf.not_nil!])
          end
      ...
      cached_graph_entry = graph_cache_key ? ffn_resident_graph_cache_lookup(graph_cache_key.not_nil!) : nil
      ...
      unless cached_graph_entry
        ...
        expert_chunks.each do |chunk|
          expert = chunk.expert
          batch = chunk.batch
          chunk_detail = "expert=#{expert} chunk=#{chunk.chunk_index} batch=#{batch}"
          return nil unless ffn_resident_step_ok?(Gemma4Metal.encode_gather_rows_by_map_to_buffer(enc, moe_in_buf, chunk.gather_map_buf, chunk.input_buf, row_count, batch, hp.n_embd), il, "expert_gather", chunk_detail)

          down_qw = expert_down_qw(lw, hp, expert)
          if combined_gate_up_enabled
            gate_up_qw = expert_gate_up_qw(lw, hp, expert)
            gate_up_buf = chunk.gate_up_buf || raise ArgumentError.new("ffn resident gate_up buffer missing")
            return nil unless ffn_resident_step_ok?(Qwen35Metal.encode_matmul_to_buffer(enc, gate_up_qw, chunk.input_buf, gate_up_buf, batch), il, "expert_gate_up_combined", chunk_detail)
            return nil unless ffn_resident_step_ok?(Gemma4Metal.encode_gelu_mul_split_to_buffer(enc, gate_up_buf, chunk.hidden_buf, batch, hp.expert_ff), il, "expert_gelu_split", chunk_detail)
          else
            gate_buf = chunk.gate_buf || raise ArgumentError.new("ffn resident gate buffer missing")
            up_buf = chunk.up_buf || raise ArgumentError.new("ffn resident up buffer missing")
            gate_qw = expert_gate_qw(lw, hp, expert)
            up_qw = expert_up_qw(lw, hp, expert)
            return nil unless ffn_resident_step_ok?(Qwen35Metal.encode_matmul_many_to_buffers(enc, [gate_qw, up_qw], chunk.input_buf, [gate_buf, up_buf], batch), il, "expert_gate_up", chunk_detail)
            return nil unless ffn_resident_step_ok?(Gemma4Metal.encode_gelu_mul_to_buffer(enc, gate_buf, up_buf, chunk.hidden_buf, batch * hp.expert_ff), il, "expert_gelu", chunk_detail)
          end
          return nil unless ffn_resident_step_ok?(Qwen35Metal.encode_matmul_to_buffer(enc, down_qw, chunk.hidden_buf, chunk.down_buf, batch), il, "expert_down", chunk_detail)
          return nil unless ffn_resident_step_ok?(Gemma4Metal.encode_scatter_rows_by_map_to_buffer(enc, chunk.down_buf, chunk.scatter_map_buf, route_rows_buf, batch, route_slot_count, hp.n_embd, partition: expert), il, "expert_scatter", chunk_detail)
        end
      ...
      if graph_cache_key && cached_graph_entry.nil?
        ffn_resident_graph_cache_store(graph_cache_key.not_nil!, graph)
      end
      if ENV["DIFFUSION_GEMMA_FFN_RESIDENT_GRAPH_STATS"]? == "1"
        stats = graph.stats
        STDERR.puts "diffusion_gemma_ffn_resident_graph layer=#{il} rows=#{row_count} active_experts=#{assignments_by_expert.size} route_slots=#{route_slot_count} ops=#{stats.n_ops} waves=#{stats.n_waves} barriers=#{stats.n_barriers} max_wave_width=#{stats.max_wave_width} graph_cache=#{graph_cache_status}"
      end
      cmd = ML::Metal::CommandBuffer.new
    ...
    def moe_expert_output_resident_graph(weights : DiffusionGemmaWeights,
                                         il : Int32,
                                         expert : Int32,
                                         ffn_in : Array(Float32)) : Array(Float32)?
    ...
    def moe_expert_down_reduce_rows_from_hidden_resident_command(weights : DiffusionGemmaWeights,
                                                                 il : Int32,
                                                                 hidden_rows_by_expert : Hash(Int32, Array(Float32)),
      ...
      hp = weights.hparams
      lw = weights.layers[il]
      raise ArgumentError.new("down reduce row_count must be positive") unless row_count > 0
      raise ArgumentError.new("down reduce selected row count mismatch") unless selected_by_row.size == row_count
      raise ArgumentError.new("down reduce route offset count mismatch") unless route_offsets_by_row.size == row_count
      raise ArgumentError.new("down expert scale size mismatch") unless lw.ffn_down_exps_scale.size == hp.expert_count

      route_slot_count = 0
      route_counts_by_row = Array(Int32).new(row_count, 0)
      selected_by_row.each_with_index do |selected, row|
        raise ArgumentError.new("down reduce routes must not be empty") if selected.empty?
        raise ArgumentError.new("down reduce route offsets must be contiguous") unless route_offsets_by_row[row] == route_slot_count
        route_counts_by_row[row] = selected.size
        route_slot_count += selected.size
      end
      raise ArgumentError.new("down reduce route slots must be positive") unless route_slot_count > 0

      route_weights = Array(Float32).new(route_slot_count, 0.0_f32)
      selected_by_row.each_with_index do |selected, row|
        selected.each_with_index do |route, route_index|
          raise ArgumentError.new("down reduce route expert out of range") if route.expert < 0 || route.expert >= hp.expert_count
          slot = route_offsets_by_row[row] + route_index
          route_weights[slot] = route.weight * lw.ffn_down_exps_scale[route.expert]
        end
      end

      scatter_maps_by_expert = {} of Int32 => Array(Int32)
      hidden_rows_by_expert.keys.sort.each do |expert|
        batch = batch_by_expert[expert]
        hidden_rows = hidden_rows_by_expert[expert]
        assignments = assignments_by_expert[expert]
        raise ArgumentError.new("down reduce expert id out of range") if expert < 0 || expert >= hp.expert_count
        raise ArgumentError.new("down reduce expert row_count must be positive") unless batch > 0
        raise ArgumentError.new("down reduce expert hidden rows size mismatch") unless hidden_rows.size == batch * hp.expert_ff
        raise ArgumentError.new("down reduce expert batch assignment mismatch") unless batch == assignments.size
        return nil if batch > Qwen35Metal::GEMM_BATCH_THRESHOLD

        scatter_maps_by_expert[expert] = Array(Int32).new(batch) do |i|
          row = assignments[i][0]
          route_index = assignments[i][1]
          raise ArgumentError.new("down reduce scatter row out of range") if row < 0 || row >= row_count
          raise ArgumentError.new("down reduce route index out of range") if route_index < 0 || route_index >= selected_by_row[row].size
          route_offsets_by_row[row] + route_index
        end
      end
      ...
      hidden_rows_by_expert.keys.sort.each do |expert|
        batch = batch_by_expert[expert]
        hidden_rows = hidden_rows_by_expert[expert]
        scatter_map = scatter_maps_by_expert[expert]
        hidden_buf = ML::MetalBuffer.from_array(hidden_rows)
        scatter_map_buf = metal_int32_buffer(scatter_map)
        down_buf = ML::MetalBuffer.new(batch.to_i64 * hp.n_embd * sizeof(Float32))
        owned_buffers.concat([hidden_buf, scatter_map_buf, down_buf])
      ...
      cmd.commit
      cmd.wait
      reduced_buf.read(row_count * hp.n_embd)
    end
    ...
    def moe_expert_rows_by_expert_resident_graph_from_row_buffer(weights : DiffusionGemmaWeights,
      ...
      lw = weights.layers[il]
      raise ArgumentError.new("expert row_count must be positive") unless row_count > 0
      raise ArgumentError.new("expert row buffer too small") if ffn_rows_buf.size < row_count.to_i64 * hp.n_embd * sizeof(Float32)
      raise ArgumentError.new("down expert scale size mismatch") unless lw.ffn_down_exps_scale.size == hp.expert_count
      ...
      assignments_by_expert.keys.sort.each do |expert|
        ...
        raise ArgumentError.new("expert id out of range") if expert < 0 || expert >= hp.expert_count
        raise ArgumentError.new("expert row_count must be positive") unless batch > 0
        raise ArgumentError.new("expert batch assignment mismatch") unless batch == assignments.size

        gather_map = Array(Int32).new(batch) do |i|
          row = assignments[i][0]
          raise ArgumentError.new("expert gather row out of range") if row < 0 || row >= row_count
          row
        end

        map_buf = metal_int32_buffer(gather_map)
    ...
    private def emit_moe_grouped_resident_fallback(route : String,
                                                   il : Int32,
                                                   row_count : Int32,
                                                   step : String,
                                                   ...
                                                   over_threshold_experts : Int32,
                                                   timing_t0 : Time::Instant?) : Nil
      return unless moe_grouped_resident_timing_enabled?
      elapsed_ms = timing_t0 ? (Time.instant - timing_t0.not_nil!).total_milliseconds : 0.0
      STDERR.puts "diffusion_gemma_moe_grouped_resident_fallback route=#{route} layer=#{il} rows=#{row_count} step=#{step} active_experts=#{active_experts} route_slots=#{route_slot_count} max_expert_batch=#{max_expert_batch} over_threshold_experts=#{over_threshold_experts} elapsed_ms=#{elapsed_ms}"
    end
    ...
    def moe_grouped_route_reduce_resident_graph_from_row_buffer(weights : DiffusionGemmaWeights,
      ...
      assignments_by_expert.keys.sort.each do |expert|
        ...
        while chunk_start < assignments.size
          ...
          chunk_assignments = assignments[chunk_start, chunk_size]
          batch = chunk_assignments.size
          raise ArgumentError.new("gpu reduce expert chunk must not be empty") unless batch > 0
      ...
      read_done = Time.instant if timing_enabled
      post_norm_t0 = Time.instant if timing_enabled
      row_count.times do |row|
    ...
    def moe_ffn_grouped_expert_rows_timed(weights : DiffusionGemmaWeights,
      ...
      if resident_rows_by_expert
        ...
        use_post_activation_reduce_command = moe_grouped_resident_post_activation_reduce_command_enabled?(row_count)
        use_resident_down_command = !use_post_activation_reduce_command && moe_grouped_resident_down_command_enabled?(row_count)
        ...
        assignments_by_expert.each do |expert, assignments|
          ...
          if use_post_activation_reduce_command || use_resident_down_command
        ...
        unless deferred_hidden_rows_by_expert.empty?
          if use_post_activation_reduce_command
            down_t0 = Time.instant
            reduced_rows = moe_expert_down_reduce_rows_from_hidden_resident_command(
              weights,
              il,
              deferred_hidden_rows_by_expert,
              selected_by_row,
              route_offsets_by_row,
              assignments_by_expert,
              batch_by_expert,
            ...
            down_ms += (Time.instant - down_t0).total_milliseconds if reduced_rows
            if reduced_rows
              norm_t0 = Time.instant
              row_count.times do |row|
                fast_rms_norm_slice!(reduced_rows, row * hp.n_embd, hp.n_embd, lw.post_ffw_norm_2, hp.rms_eps)
              end
    ...
    def layer_forward_prompt_rows_with_projections_batched_timed(weights : DiffusionGemmaWeights,
      ...
      moe_rows = if prompt_materialize_grouped_moe_enabled?
                   ...
                   moe_grouped_active_experts = grouped.active_experts
    ...
    def build_prompt_layer_cache(weights : DiffusionGemmaWeights,
      ...
      max_layers.times do |il|
        ...
        materialize_moe_grouped_active_experts_by_layer << timed_materialize.moe_grouped_active_experts
      ...
      PromptLayerCache.new(
        ...
        materialize_moe_grouped_active_experts_by_layer,
    ...
    def clear_ffn_resident_graph_cache : Nil
      @@ffn_resident_graph_cache_mutex.synchronize do
    ...
    def ffn_resident_graph_cache_stats : NamedTuple(entries: Int32, hits: Int64, misses: Int64)
      @@ffn_resident_graph_cache_mutex.synchronize do
        {
          entries: @@ffn_resident_graph_cache.size,
          hits:    @@ffn_resident_graph_cache_hits,
          misses:  @@ffn_resident_graph_cache_misses,
        }
      end
    end
    ...
    def moe_grouped_gpu_gather_enabled?(canvas_len : Int32) : Bool
      return false unless ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_GATHER"]? == "1"
      return false if ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_GATHER_OFF"]? == "1"
      return false if canvas_len < moe_grouped_gpu_gather_min_canvas
      max_canvas = moe_grouped_gpu_gather_max_canvas
      return false if max_canvas > 0 && canvas_len > max_canvas
      Qwen35Metal.available?
    end
    ...
    def moe_grouped_resident_post_activation_reduce_command_enabled?(canvas_len : Int32) : Bool
      return false unless ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_POST_ACT_REDUCE_COMMAND"]? == "1"
      return false if ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_POST_ACT_REDUCE_COMMAND_OFF"]? == "1"
      return false if moe_grouped_resident_graph_enabled?
      return false if canvas_len < moe_grouped_resident_post_activation_reduce_command_min_canvas
      max_canvas = moe_grouped_resident_post_activation_reduce_command_max_canvas
      return false if max_canvas > 0 && canvas_len > max_canvas
      Qwen35Metal.available?
    end

    def moe_grouped_resident_post_activation_reduce_command_min_canvas : Int32
      env_i32("DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_POST_ACT_REDUCE_COMMAND_MIN_CANVAS", 8)
    end

    def moe_grouped_resident_post_activation_reduce_command_max_canvas : Int32
      env_i32("DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_POST_ACT_REDUCE_COMMAND_MAX_CANVAS", 8)
    end
    ...
    private def build_grouped_moe_inputs_by_expert(ffn_in_rows : Array(Float32),
                                                   assignments_by_expert : Hash(Int32, Array(Tuple(Int32, Int32))),
                                                   hidden_dim : Int32) : NamedTuple(inputs: Hash(Int32, Array(Float32)), prep_ms: Float64)
      inputs = {} of Int32 => Array(Float32)
      prep_ms = 0.0
      assignments_by_expert.each do |expert, assignments|
        batch = assignments.size
        gather_t0 = Time.instant
        ffn_inputs = Array(Float32).new(batch * hidden_dim, 0.0_f32)
        assignments.each_with_index do |assignment, batch_row|
          row = assignment[0]
          src_offset = row * hidden_dim
          dst_offset = batch_row * hidden_dim
          (ffn_inputs.to_unsafe + dst_offset).copy_from(ffn_in_rows.to_unsafe + src_offset, hidden_dim)
        end
        prep_ms += (Time.instant - gather_t0).total_milliseconds
        inputs[expert] = ffn_inputs
      end
      {inputs: inputs, prep_ms: prep_ms}
    end

    private def metal_int32_buffer(values : Array(Int32)) : ML::MetalBuffer
    ...
    private def ffn_resident_graph_cache_key(model_id : UInt64,
                                             il : Int32,
                                             row_count : Int32,
                                             hidden_dim : Int32,
                                             shared_ff : Int32,
      ...
      String.build do |io|
        ...
        io << "|shared_ff=" << shared_ff
        io << "|expert_ff=" << expert_ff
        io << "|slots=" << route_slot_count
        io << "|chunk=" << max_expert_batch
        io << "|combined=" << (combined_gate_up ? 1 : 0)
        io << "|canvas=" << (canvas ? 1 : 0)
        assignments_by_expert.keys.sort.each do |expert|
          assignments = assignments_by_expert[expert]
          io << "|e=" << expert << ":"
          chunk_start = 0
    ...
    private def ffn_resident_graph_cache_max_entries : Int32
      raw = ENV["DIFFUSION_GEMMA_FFN_RESIDENT_GRAPH_CACHE_MAX_ENTRIES"]?
      value = raw ? raw.to_i? : nil
      max_entries = value || 32
      max_entries < 1 ? 1 : max_entries
    end

    private def ffn_resident_graph_cache_lookup(key : String) : FfnResidentGraphCacheEntry?
      @@ffn_resident_graph_cache_mutex.synchronize do
        if entry = @@ffn_resident_graph_cache[key]?
          @@ffn_resident_graph_cache_hits += 1
          entry
        else
          @@ffn_resident_graph_cache_misses += 1
          nil
        end
      end
    end

    private def ffn_resident_graph_cache_store(key : String,
                                               graph : ML::Metal::ComputeGraph) : FfnResidentGraphCacheEntry
      entry = FfnResidentGraphCacheEntry.new(graph, graph.stats)
      @@ffn_resident_graph_cache_mutex.synchronize do
        if cached = @@ffn_resident_graph_cache[key]?
          cached
        else
