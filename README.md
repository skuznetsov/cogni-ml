...
| `QWEN35_CONSTRAINED_TOOL_CALL_PREFIX=1` | Experimental greedy-only structured tool-call prefix mode. Requires `QWEN35_TOOLS_JSON`; constrains generation to Qwen XML `<tool_call>\n<function=...>\n` prefixes over the available function names and the first required `<parameter=...>\n` tag, then falls back for free-form values. |
