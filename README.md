...
| `QWEN35_ALLOWED_HEAD_CPU_MAX=N` | Default-off constrained-head policy hook. For `forward_top1_allowed`, use Metal body hidden readback plus CPU selected-row projection when `allowed_ids.size <= N` instead of the Q6 allowed-head kernel. Raspberry Pi 5 resident-Q6 calibration currently treats `N=7` as near-boundary and `N=12` as the conservative unbatched clear-route threshold; keep configurable. |
