...
| `QWEN35_CONSTRAINED_TOOL_CALL_PREFIX=1` | Experimental greedy-only structured tool-call mode. Requires `QWEN35_TOOLS_JSON`; constrains Qwen XML `<tool_call>\n<function=...>\n` prefixes over the available function names, the first required `<parameter=...>\n` tag, then falls back for the free-form value and constrains `</parameter>\n</function>\n</tool_call>` after a non-empty single-line value emits a newline. |
