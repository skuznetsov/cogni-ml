...
| `QWEN35_ALLOWED_HEAD_CPU_MAX=N` | Default-off constrained-head policy hook. For `forward_top1_allowed`, use Metal body hidden readback plus CPU selected-row projection when `allowed_ids.size <= N` instead of the Q6 allowed-head kernel. Current Raspberry Pi 5 resident-Q6 probes suggest `N=7`; keep configurable. |
