...
| `QWEN35_METAL_PROFILE=1` | Enable Metal dispatch/profile attribution for the timed decode region in `qwen35_generate`. The report includes wave/group timings, prefill/source-replay phase traces, route markers, matmul logical traffic, conversion traffic, and CPU fallback counts. |
...
| `QWEN35_CONSTRAINT_FRONTIER_TRACE=1` | Emit diagnostic `constraint frontier trace` rows before runtime constrained allowed-head decisions, including stage, position, allowed count, and `ids_csv`. Deterministic forced spans are not counted as allowed-head frontier decisions. |
| `QWEN35_ALLOWED_HEAD_CPU_MAX=N` | Default-off constrained-head policy hook. For `forward_top1_allowed`, use Metal body hidden readback plus CPU selected-row projection when `allowed_ids.size <= N` instead of the Q6 allowed-head kernel. Raspberry Pi 5 resident-Q6 calibration treats `N=7` as the warmed resident-probe threshold, while `N=12` remains the conservative unbatched product default until the V3D adapter owns the same warmed corridor; keep configurable. |
| `QWEN35_ALLOWED_HEAD_Q6_OFF=1` | Disable the Q6 allowed-head fast route and force the exact hidden-readback/CPU selected-row fallback. This is the fail-closed kill switch for the current Metal Q6 route and future resident Q6 head adapters. |
| `QWEN35_ALLOWED_HEAD_CAPTURE_PATH=/path.jsonl` | Diagnostic capture hook for constrained allowed-head routing. When set, `forward_top1_allowed` forces the exact hidden-readback/selected-row path, appends one JSONL row per allowed-head decision with `pos`, `input_token_id`, `allowed_ids`, and pre-output-norm `hidden`, then returns the same exact selected-row top1. This is for RPi5/V3D replay probes and is default-off. With `QWEN35_METAL_PROFILE=1`, route markers distinguish `allowed_head.metal_q6`, `allowed_head.q6_off`, `allowed_head.cpu_threshold`, and `allowed_head.capture`. |
