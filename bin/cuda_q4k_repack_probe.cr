...
begin
  ...
  modules.concat([raw_mod, repack_mod])
  ...
  buffers.concat([d_raw, d_qvals, d_scales, d_mins, d_x, d_raw_out, d_repack_out])
  ...
  puts "speedup=#{(raw_ms / repack_ms_gpu).round(4)}"
  ...
  puts "ok=#{raw_cos >= 0.99999 && raw_max <= 1.0e-3_f32 && repack_cos >= 0.99999 && repack_max <= 1.0e-3_f32}"
ensure
  buffers.each(&.close)
  modules.each(&.close)
  ctx.close
