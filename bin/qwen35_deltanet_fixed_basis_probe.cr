...
module ProbeRuntime
  ...
  @@self_spec_tail_salvage_max_corrections = 0
  ...
  def self.self_spec_tail_salvage_max_corrections : Int32
    @@self_spec_tail_salvage_max_corrections
  end

  def self.self_spec_tail_salvage_max_corrections=(count : Int32)
    raise "self-spec tail-salvage max corrections must be non-negative" if count < 0
    @@self_spec_tail_salvage_max_corrections = count
  end
...
private def simulate_self_spec_cost_truth_table(weights : ML::GGUF::Qwen35Weights,
  ...
  print_cost_truth_row("draft", "lowrank_gpu_chain", chain[:steps], chain[:chain_ms], plain_per_token, chain_match,
    " agreement=#{chain[:agreement]}/#{chain[:steps]} lcs_agreement=#{chain[:agreement_lcs]}/#{chain[:steps]} first_mismatch=#{chain[:first_mismatch]} salvage_accepted=#{chain[:salvage_accepted]}/#{chain[:steps]} salvage_corrections=#{chain[:salvage_corrections]} salvage_dropped=#{chain[:salvage_dropped]} exact_ms=#{chain[:exact_ms].round(3)} note=autoregressive_top1_id_chain")
  ...
  print_cost_truth_row("draft", "pca_updown_gpu_chain", updown_chain[:steps], updown_chain[:chain_ms], plain_per_token, updown_match,
    " agreement=#{updown_chain[:agreement]}/#{updown_chain[:steps]} lcs_agreement=#{updown_chain[:agreement_lcs]}/#{updown_chain[:steps]} first_mismatch=#{updown_chain[:first_mismatch]} salvage_accepted=#{updown_chain[:salvage_accepted]}/#{updown_chain[:steps]} salvage_corrections=#{updown_chain[:salvage_corrections]} salvage_dropped=#{updown_chain[:salvage_dropped]} updown_rank=#{updown_chain[:updown_rank]} exact_ms=#{updown_chain[:exact_ms].round(3)} note=autoregressive_top1_id_chain")
end
...
private def token_lcs_length(a : Array(Int32), b : Array(Int32)) : Int32
  prev = Array(Int32).new(b.size + 1, 0)
  curr = Array(Int32).new(b.size + 1, 0)
  a.each do |av|
    b.each_with_index do |bv, j|
      curr[j + 1] = av == bv ? prev[j] + 1 : Math.max(prev[j + 1], curr[j])
    end
    prev, curr = curr, prev
    curr.fill(0)
  end
...
private def first_mismatch_index(a : Array(Int32), b : Array(Int32)) : Int32
  limit = Math.min(a.size, b.size)
  limit.times do |i|
    return i.to_i32 unless a[i] == b[i]
  end
  a.size == b.size ? -1 : limit.to_i32
end

private def token_tail_salvage_oracle(draft : Array(Int32), exact : Array(Int32)) : NamedTuple(accepted: Int32, corrections: Int32, dropped: Int32)
  i = 0
  j = 0
  accepted = 0
  corrections = 0
  ...
  while i < draft.size && j < exact.size
    if draft[i] == exact[j]
      accepted += 1
      i += 1
      j += 1
    ...
    if target = realign_at
      corrections += target - j
      j = target
    else
      dropped += 1
      i += 1
  ...
  {accepted: accepted, corrections: corrections, dropped: dropped}
end
...
private def simulate_self_draft_gpu_chain_run(weights : ML::GGUF::Qwen35Weights,
                                              ...
                                              ffn_updown_adapters : FFNUpDownAdapterMap? = nil,
                                              draft_updown_layer_indices : Set(Int32)? = nil,
                                              capture_top2 : Bool = false,
                                              exact_first : Int32 = 0) : NamedTuple(steps: Int32, exact_first: Int32, draft_steps: Int32, submit_ms: Float64, wait_ms: Float64, chain_ms: Float64, exact_ms: Float64, agreement: Int32, chain_ids: Array(Int32), chain_second_ids: Array(Int32), chain_top2_margins: Array(Float64), exact_ids: Array(Int32), agreement_lcs: Int32, top2_agreement: Int32, top2_rescues: Int32, salvage_accepted: Int32, salvage_corrections: Int32, salvage_dropped: Int32, first_mismatch: Int32, updown_rank: Int32)
  ...
  exact_first = Math.min(Math.max(exact_first, 0), n_draft)
  draft_steps = n_draft - exact_first
  hp = weights.hparams
  h_k = hp.ssm_group_count
  h_v = hp.ssm_time_step_rank
  s = hp.ssm_state_size
  ...
  chain_tok = first_token
  exact_first.times do |i|
    out = ML::GGUF::Qwen35CPU.forward_top1(weights, chain_tok, calib_count + i, chain_state)
    chain_ids << out[0]
    chain_second_ids << -1 if capture_top2
    chain_top2_margins << 0.0 if capture_top2
    chain_tok = out[0]
  end
  chain_lr = build_lr_states.call(chain_state)
  initial_token_buf = ML::MetalBuffer.new(sizeof(UInt32).to_i64)
  initial_token_buf.contents.as(Pointer(UInt32)).value = chain_tok.to_u32
  ...
  draft_steps.times do |i|
    t0 = Time.instant
    pos = calib_count + exact_first + i
    sub = if capture_top2
            ML::GGUF::Qwen35CPU.forward_self_draft_top2_from_token_buf_async(weights, token_buf, 0, pos, chain_state,
            ...
            ML::GGUF::Qwen35CPU.forward_self_draft_top1_from_token_buf_async(weights, token_buf, 0, pos, chain_state,
              lowrank_set, chain_lr, shared_basis_bufs, rank,
              lowrank_updown_x_mean_bufs: updown_x_mean_bufs,
              lowrank_updown_c_mean_bufs: updown_c_mean_bufs,
              lowrank_updown_coeff_w_bufs: updown_coeff_w_bufs,
  ...
  salvage = token_tail_salvage_oracle(chain_ids, exact_ids)
...

  {
    ...
    salvage_accepted: salvage[:accepted],
    salvage_corrections: salvage[:corrections],
...
private def simulate_self_spec_gpu_pipeline_run(weights : ML::GGUF::Qwen35Weights,
  ...
  tail_salvage_max_corrections = ProbeRuntime.self_spec_tail_salvage_max_corrections
  tail_salvage_enabled = tail_salvage_max_corrections > 0
  if tail_salvage_enabled
    raise "GPU pipeline tail salvage requires verifier backup" unless use_verifier_backup
    if tree2_first || tree2_anywhere || tree2_staged_tokens > 0 || !tree2_margin_guard.nil? || !tree2_branch_guard.nil? || !risk_offramp_margin.nil? || mtp_k2_on_reject || reject_offramp_after > 0
      raise "GPU pipeline tail salvage currently supports only the plain self-draft route"
    end
  end
  ...
  branch_guard_snapshot_suffix_allowed = ->(block : GpuDraftBlock, suffix_start : Int32, verifier_size : Int32) {
    ...
    if threshold_value = selected_threshold
      ...
      suffix_end = Math.min(verifier_size, block.size) - 1
  ...
  tail_salvage_scratch = if tail_salvage_enabled
                           scratch = ML::GGUF::Qwen35CPU::State.new(hp, max_seq: max_seq)
                           ML::GGUF::Qwen35CPU.prepare_state_metal!(scratch, hp)
                           scratch
                         else
                           nil
                         end
  ...
  while emitted_tokens < gen_tokens
    ...
    if tail_salvage_enabled && rejected && emitted_tokens < gen_tokens
      tail_salvage_checks += 1
      t_salvage = Time.instant
      scratch = tail_salvage_scratch.not_nil!
      backup = verifier_backup.not_nil!
      copy_verifier_state.call(scratch, backup, cycle_start_pos)
      ...
      while draft_index < salvage_candidates.size && emitted_tokens < gen_tokens
        ...
        if cand == local_expected
          ...
          pos_last = pos
          if emitted_tokens < gen_tokens
            local_expected = ML::GGUF::Qwen35CPU.forward_top1(weights, cand, pos, scratch)[0]
          end
          draft_index += 1
        elsif local_corrections < tail_salvage_max_corrections
          exact_token = local_expected
          exact_ids << exact_token
          correction_or_accepted << exact_token
          emitted_ids << exact_token
          emitted_tokens += 1
          local_corrections += 1
          pos = cycle_start_pos + correction_or_accepted.size - 1
          last_token = exact_token
          pos_last = pos
          if emitted_tokens < gen_tokens
            local_expected = ML::GGUF::Qwen35CPU.forward_top1(weights, exact_token, pos, scratch)[0]
          end
        else
          local_dropped += 1
          draft_index += 1
      ...
      if attr_collect
        verifier_prefill_ms += dt_salvage
        verifier_chunks += 1
        verifier_tokens_count += local_accepted + local_corrections
      end
      wba.try(&.mark("verifier", "tail_salvage_#{chunks}_#{rejected_index}", t_salvage, Time.instant))
    end
  ...
  while serial_emitted_tokens < gen_tokens
    ...
    if tail_salvage_enabled && rejected && serial_emitted_tokens < gen_tokens
      backup = serial_backup.not_nil!
      scratch = ML::GGUF::Qwen35CPU::State.new(hp, max_seq: max_seq)
      ML::GGUF::Qwen35CPU.prepare_state_metal!(scratch, hp)
      copy_verifier_state.call(scratch, backup, cycle_start_pos)
      ...
      while draft_index < salvage_candidates.size && serial_emitted_tokens < gen_tokens
        ...
        if cand == local_expected
          ...
          correction_or_accepted << cand
          serial_emitted_ids << cand
          serial_emitted_tokens += 1
          serial_last_token = cand
          serial_pos_last = cycle_start_pos + correction_or_accepted.size - 1
          if serial_emitted_tokens < gen_tokens
            local_expected = ML::GGUF::Qwen35CPU.forward_top1(weights, cand, serial_pos_last, scratch)[0]
          end
          draft_index += 1
        elsif local_corrections < tail_salvage_max_corrections
          exact_token = local_expected
          serial_exact_ids << exact_token
          correction_or_accepted << exact_token
          serial_emitted_ids << exact_token
          serial_emitted_tokens += 1
          local_corrections += 1
          serial_last_token = exact_token
          serial_pos_last = cycle_start_pos + correction_or_accepted.size - 1
          if serial_emitted_tokens < gen_tokens
            local_expected = ML::GGUF::Qwen35CPU.forward_top1(weights, exact_token, serial_pos_last, scratch)[0]
          end
        else
          draft_index += 1
        end
  ...
  end.join(";")
  {
    ...
    tail_salvage_max_corrections: tail_salvage_max_corrections,
    ...
    tail_salvage_verify_ms:       tail_salvage_verify_ms,
...
simulate_self_draft_gpu_chain_exact_first = 0
...
OptionParser.parse(ARGV) do |p|
  ...
  p.on("--simulate-self-draft-gpu-chain-exact-first=N", "With --simulate-self-draft-gpu-chain, emit N exact greedy tokens before starting the low-rank draft chain") { |v| simulate_self_draft_gpu_chain_exact_first = v.to_i }
  ...
  p.on("--simulate-self-spec-gpu-pipeline-tail-salvage=N", "Default-off plain-route controller: after a reject, emit up to N exact corrections while trying to realign and reuse the draft tail") { |v| ProbeRuntime.self_spec_tail_salvage_max_corrections = v.to_i }
  ...
  if simulate_logit_layers.empty?
    ...
    if simulate_self_draft_gpu_chain > 0
      chain = simulate_self_draft_gpu_chain_run(weights, token_ids, calib_count, simulate_self_draft_gpu_chain, layer_bases, rank,
        capture_top2: simulate_self_draft_gpu_chain_top2, exact_first: simulate_self_draft_gpu_chain_exact_first)
      exact_first_note = chain[:exact_first] > 0 ? " exact_first=#{chain[:exact_first]} draft_steps=#{chain[:draft_steps]}" : ""
      puts "self_draft_gpu_chain layers=#{simulate_logit_layers.join(',')} rank=#{rank} steps=#{chain[:steps]}#{exact_first_note} submit_ms=#{chain[:submit_ms].round(3)} wait_ms=#{chain[:wait_ms].round(3)} chain_ms=#{chain[:chain_ms].round(3)} exact_ms=#{chain[:exact_ms].round(3)} agreement=#{chain[:agreement]}/#{chain[:steps]} lcs_agreement=#{chain[:agreement_lcs]}/#{chain[:steps]} first_mismatch=#{chain[:first_mismatch]} top2_agreement=#{chain[:top2_agreement]}/#{chain[:steps]} top2_rescues=#{chain[:top2_rescues]} salvage_accepted=#{chain[:salvage_accepted]}/#{chain[:steps]} salvage_corrections=#{chain[:salvage_corrections]} salvage_dropped=#{chain[:salvage_dropped]} chain_ids=#{chain[:chain_ids].join(',')} exact_ids=#{chain[:exact_ids].join(',')}"
      if simulate_self_draft_gpu_chain_text
        puts "self_draft_gpu_chain_text layers=#{simulate_logit_layers.join(',')} rank=#{rank} steps=#{chain[:steps]}#{exact_first_note} agreement=#{chain[:agreement]}/#{chain[:steps]} lcs_agreement=#{chain[:agreement_lcs]}/#{chain[:steps]} first_mismatch=#{chain[:first_mismatch]} top2_agreement=#{chain[:top2_agreement]}/#{chain[:steps]} top2_rescues=#{chain[:top2_rescues]} salvage_accepted=#{chain[:salvage_accepted]}/#{chain[:steps]} salvage_corrections=#{chain[:salvage_corrections]} salvage_dropped=#{chain[:salvage_dropped]} draft_text=#{tok.decode(chain[:chain_ids]).inspect} exact_text=#{tok.decode(chain[:exact_ids]).inspect}"
      end
      if chain_updown_rank = simulate_self_draft_gpu_chain_updown_rank
        ...
        updown_chain = simulate_self_draft_gpu_chain_run(weights, token_ids, calib_count, simulate_self_draft_gpu_chain, layer_bases, rank,
          ...
          exact_first: simulate_self_draft_gpu_chain_exact_first)
        updown_layer_note = chain_updown_layer_set ? " updown_layers=#{chain_updown_layer_set.not_nil!.to_a.sort.join(',')}" : ""
        updown_exact_first_note = updown_chain[:exact_first] > 0 ? " exact_first=#{updown_chain[:exact_first]} draft_steps=#{updown_chain[:draft_steps]}" : ""
        puts "self_draft_gpu_chain_updown layers=#{simulate_logit_layers.join(',')} rank=#{rank} updown_rank=#{updown_chain[:updown_rank]}#{updown_layer_note} steps=#{updown_chain[:steps]}#{updown_exact_first_note} submit_ms=#{updown_chain[:submit_ms].round(3)} wait_ms=#{updown_chain[:wait_ms].round(3)} chain_ms=#{updown_chain[:chain_ms].round(3)} exact_ms=#{updown_chain[:exact_ms].round(3)} agreement=#{updown_chain[:agreement]}/#{updown_chain[:steps]} lcs_agreement=#{updown_chain[:agreement_lcs]}/#{updown_chain[:steps]} first_mismatch=#{updown_chain[:first_mismatch]} top2_agreement=#{updown_chain[:top2_agreement]}/#{updown_chain[:steps]} top2_rescues=#{updown_chain[:top2_rescues]} salvage_accepted=#{updown_chain[:salvage_accepted]}/#{updown_chain[:steps]} salvage_corrections=#{updown_chain[:salvage_corrections]} salvage_dropped=#{updown_chain[:salvage_dropped]} chain_ids=#{updown_chain[:chain_ids].join(',')} exact_ids=#{updown_chain[:exact_ids].join(',')}"
        if simulate_self_draft_gpu_chain_text
          puts "self_draft_gpu_chain_updown_text layers=#{simulate_logit_layers.join(',')} rank=#{rank} updown_rank=#{updown_chain[:updown_rank]}#{updown_layer_note} steps=#{updown_chain[:steps]}#{updown_exact_first_note} agreement=#{updown_chain[:agreement]}/#{updown_chain[:steps]} lcs_agreement=#{updown_chain[:agreement_lcs]}/#{updown_chain[:steps]} first_mismatch=#{updown_chain[:first_mismatch]} top2_agreement=#{updown_chain[:top2_agreement]}/#{updown_chain[:steps]} top2_rescues=#{updown_chain[:top2_rescues]} salvage_accepted=#{updown_chain[:salvage_accepted]}/#{updown_chain[:steps]} salvage_corrections=#{updown_chain[:salvage_corrections]} salvage_dropped=#{updown_chain[:salvage_dropped]} draft_text=#{tok.decode(updown_chain[:chain_ids]).inspect} exact_text=#{tok.decode(updown_chain[:exact_ids]).inspect}"
        end
      end
    end
    ...
    if pipeline_route_active
      ...
      exact_refresh_note += " tail_salvage=#{ProbeRuntime.self_spec_tail_salvage_max_corrections}" if ProbeRuntime.self_spec_tail_salvage_max_corrections > 0
