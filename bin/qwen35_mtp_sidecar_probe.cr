...
mtp_spec_wall_top2_on_reject = false
mtp_spec_wall_top2_miss_offramp = false
mtp_spec_wall_top2_rescue_continue = false
mtp_spec_wall_promote_top2_margin = nil.as(Float64?)
mtp_spec_wall_min_margin = nil.as(Float64?)
mtp_spec_wall_min_margin_offramp = false
mtp_spec_wall_entry_skip_always = false
mtp_spec_wall_entry_skip_prompt_tokens_lt = nil.as(Float64?)
mtp_spec_wall_router_trace_path = nil.as(String?)
mtp_spec_wall_router_thresholds = [] of Float64
mtp_spec_wall_raw_blend = nil.as(Float32?)
mtp_spec_wall_profile = false
mtp_spec_wall_serial_early_verify = false
mtp_spec_wall_snapshot_cost_probe = false
mtp_wall_exact_warmup = 2_i32
mtp_spec_wall_resync_draft_hidden = false
mtp_spec_wall_rec_checkpoint_replay = false
mtp_spec_wall_rec_rollback_log = false
mtp_spec_wall_state_slot_probe = false
mtp_spec_wall_exact_first = false
mtp_spec_wall_gpu_fallback_chain = false
...
class MtpSpecWallAggregate
  ...
  property rejected_tail_tokens : Int32
  property replay_tokens : Int32
  property fallback_tokens : Int32
  property snapshot_tokens : Int32
  property state_slot_skipped_backups : Int32
  property state_slot_recoveries : Int32
  ...
  property margin_skips : Int32
  property entry_skips : Int32
  property entry_skip_tokens : Int32
  property mtp_ms_sum : Float64
  ...
  property persistent_setup_ms_sum : Float64
  ...
  def initialize
    ...
    @verifier_calls = 0
    @verifier_tokens = 0
    @rejected_tail_tokens = 0
    @replay_tokens = 0
    @fallback_tokens = 0
    @snapshot_tokens = 0
    @state_slot_skipped_backups = 0
    @state_slot_recoveries = 0
    @top2_checks = 0
    @top2_rescues = 0
...
class MtpSpecWallRouterPass
  ...
  getter verifier_tokens_delta : Int32
  getter rejected_tail_tokens_delta : Int32
  ...
  def initialize(@pass_index, @start_i, @end_i, @wall_before_ms, @wall_after_ms,
                 ...
                 @mtp_delta_ms, @verifier_delta_ms, @verifier_tokens_delta,
                 @rejected_tail_tokens_delta, @replay_delta_ms, @fallback_delta_ms,
...
private def mtp_sync_state_token!(weights, mtp : ML::GGUF::Qwen35MTPWeights,
                                  prev_hidden, token_id : Int32, pos : Int32,
                                  mtp_state : ML::GGUF::Qwen35MTP::State) : Float64
  start = Time.instant
  ML::GGUF::Qwen35MTP.forward_one_hidden(weights, mtp, prev_hidden, token_id, pos, normalized: true, mtp_state: mtp_state)
  elapsed_ms(start)
end

private def mtp_sync_state_token!(weights, mtp : ML::GGUF::Qwen35GGUFMTPWeights,
                                  prev_hidden, token_id : Int32, pos : Int32,
                                  mtp_state : ML::GGUF::Qwen35MTP::State) : Float64
  start = Time.instant
  ML::GGUF::Qwen35MTP.forward_one_hidden_gguf(weights, mtp, prev_hidden, token_id, pos, mtp_state)
  elapsed_ms(start)
end
...
private def append_exact_fallback_suffix!(weights, state, ids, token : Int32, pos : Int32, target_size : Int32, gpu_chain : Bool, gpu_chain_min : Int32) : {Int32, Int32, Int32}
  remaining = target_size - ids.size
  return {token, pos, 0} if remaining <= 0

  if gpu_chain && remaining >= gpu_chain_min
    if chain = ML::GGUF::Qwen35CPU.forward_top1_chain_gpu(weights, token, pos, state, remaining)
      chain.each { |id| ids << id }
      return {chain[-1], pos + chain.size, chain.size}
    end
  end

  emitted = 0
  while ids.size < target_size
    next_id, _next_logit = ML::GGUF::Qwen35CPU.forward_top1(weights, token, pos, state)
    ids << next_id
    token = next_id
    pos += 1
    emitted += 1
...
private def mtp_wall_entry_skip?(always : Bool,
                                 prompt_tokens_lt : Float64?,
                                 features : PromptEntryFeatures) : Bool
  return true if always
  if threshold = prompt_tokens_lt
    return true if features.prompt_tokens.to_f64 < threshold
  end
  false
end
...
private def write_router_trace(io : IO, label : String, gamma : Int32, pass : MtpSpecWallRouterPass,
  ...
  JSON.build(io) do |json|
    json.object do
      ...
      json.field "verifier_tokens_delta", pass.verifier_tokens_delta
      json.field "rejected_tail_tokens_delta", pass.rejected_tail_tokens_delta
...
OptionParser.parse do |p|
  ...
  p.on("--mtp-spec-wall-top2-on-reject", "Draft MTP top1 only, then compute MTP top2 lazily only for rejected candidates") { mtp_spec_wall_top2_on_reject = true }
  ...
  p.on("--mtp-spec-wall-top2-rescue-continue", "If rejected MTP top1 has the exact correction as MTP top2, continue speculation from the exact correction boundary instead of triggering reject off-ramp") { mtp_spec_wall_top2_rescue_continue = true }
  ...
  p.on("--mtp-spec-wall-min-margin F", "Skip MTP verifier and advance exact decode when MTP top1-top2 margin is below F; requires --mtp-spec-wall-lazy-draft") { |v| mtp_spec_wall_min_margin = v.to_f64 }
  p.on("--mtp-spec-wall-min-margin-offramp", "After a low-MTP-margin skip, finish the remaining span with exact decode") { mtp_spec_wall_min_margin_offramp = true }
  p.on("--mtp-spec-wall-entry-skip-always", "Default-off diagnostic: never enter MTP wall loop; exact-decode the remaining suffix at the pass boundary") { mtp_spec_wall_entry_skip_always = true }
  p.on("--mtp-spec-wall-entry-skip-prompt-tokens-lt N", "Default-off pre-entry gate: exact-decode instead of entering MTP when prompt token count is below N") { |v| mtp_spec_wall_entry_skip_prompt_tokens_lt = v.to_f64 }
  ...
  p.on("--mtp-spec-wall-raw-blend F", "Blend previous draft hidden toward raw MTP hidden before drafting the next token") { |v| mtp_spec_wall_raw_blend = v.to_f32 }
  ...
  p.on("--mtp-spec-wall-resync-draft-hidden", "After a fully accepted lazy verifier stage, seed the next MTP stage from exact target hidden instead of prior MTP hidden") { mtp_spec_wall_resync_draft_hidden = true }
  p.on("--mtp-spec-wall-rec-checkpoint-replay", "For staged verifier chunks, checkpoint recurrent state after row 0 and use it to avoid replay on first-row reject") { mtp_spec_wall_rec_checkpoint_replay = true }
  p.on("--mtp-spec-wall-rec-rollback-log", "Log compact DeltaNet rollback data for row-1 rejects instead of copying full SSM checkpoint") { mtp_spec_wall_rec_rollback_log = true }
  p.on("--mtp-spec-wall-state-slot-probe", "Default-off state-indirection probe: skip stage-2 pre-verifier backup when checkpoint/mutated verifier state can recover the accepted boundary") { mtp_spec_wall_state_slot_probe = true }
  ...
  p.on("--mtp-spec-wall-no-exact-first", "Disable exact-first even when a preset enables it") { mtp_spec_wall_exact_first = false }
...
if run_forward && mtp_spec_wall_state_slot_probe
  abort "--mtp-spec-wall-state-slot-probe requires --mtp-spec-wall-stage 2" unless mtp_spec_wall_stage == 2
  abort "--mtp-spec-wall-state-slot-probe is incompatible with --mtp-spec-wall-serial-early-verify" if mtp_spec_wall_serial_early_verify
  abort "--mtp-spec-wall-state-slot-probe is incompatible with --mtp-spec-wall-rec-rollback-log" if mtp_spec_wall_rec_rollback_log
...
abort "--mtp-spec-wall-min-margin requires --mtp-spec-wall-lazy-draft" if mtp_spec_wall_min_margin && !mtp_spec_wall_lazy_draft
abort "--mtp-spec-wall-min-margin-offramp requires --mtp-spec-wall-min-margin" if mtp_spec_wall_min_margin_offramp && !mtp_spec_wall_min_margin
abort "--mtp-spec-wall-min-margin must be non-negative" if mtp_spec_wall_min_margin && mtp_spec_wall_min_margin.not_nil! < 0.0
abort "--mtp-spec-wall-entry-skip-prompt-tokens-lt must be non-negative" if mtp_spec_wall_entry_skip_prompt_tokens_lt && mtp_spec_wall_entry_skip_prompt_tokens_lt.not_nil! < 0.0
...
abort "--mtp-spec-wall-gpu-fallback-chain-min must be positive" if mtp_spec_wall_gpu_fallback_chain_min <= 0
abort "--mtp-wall-exact-warmup must be non-negative" if mtp_wall_exact_warmup < 0
abort "--mtp-spec-wall-raw-blend must be between 0 and 1" if mtp_spec_wall_raw_blend && (mtp_spec_wall_raw_blend.not_nil! < 0.0_f32 || mtp_spec_wall_raw_blend.not_nil! > 1.0_f32)
abort "--mtp-spec-wall-persistent-mtp-state requires --mtp-spec-wall-lazy-draft" if mtp_spec_wall_persistent_mtp_state && !mtp_spec_wall_lazy_draft
...
rows.each do |label, prompt_text|
  ...
  prompt_mtp_state = nil.as(ML::GGUF::Qwen35MTP::State?)
  prompt_mtp_state_ms = 0.0_f64
  if mtp_spec_wall_persistent_mtp_state
  ...
  if mtp_wall_exact_warmup > 0
    warm_tokens = Math.min(mtp_wall_exact_warmup, mtp_chain_tokens)

    # Keep cold Metal pipeline/setup tax out of plain-vs-spec wall timings.
    # Both warmup states are disposable and never affect measured decode state.
    warm_state = state.fork
    warm_prev = y1
    warm_pos = token_ids.size
    warm_tokens.times do
      warm_next, _ = ML::GGUF::Qwen35CPU.forward_top1(weights, warm_prev, warm_pos, warm_state)
      warm_prev = warm_next
      warm_pos += 1
    end

    warm_verify_state = state.fork
    warm_verify_tokens = Array(Int32).new(warm_tokens) { y1 }
    ML::GGUF::Qwen35CPU.prefill_tokens_hidden_top1s(weights, warm_verify_tokens, token_ids.size, warm_verify_state)
  end
  ...
  mtp_spec_wall_gammas.each do |gamma|
    ...
    wall_state_slot_skipped_backups = 0
    wall_state_slot_recoveries = 0
    wall_exact_first_tokens = 0
    wall_exact_first_ms = 0.0_f64
    wall_top2_checks = 0
    wall_top2_rescues = 0
    ...
    wall_margin_skips = 0
    wall_entry_skips = 0
    wall_entry_skip_tokens = 0
    wall_mtp_state = prompt_mtp_state.try(&.fork)
    ...
    while wall_ids.size < mtp_chain_tokens
      ...
      pass_verifier_tokens_before = wall_verifier_tokens
      pass_rejected_tail_tokens_before = wall_rejected_tail_tokens
      ...
      if mtp_wall_entry_skip?(mtp_spec_wall_entry_skip_always, mtp_spec_wall_entry_skip_prompt_tokens_lt, entry_features)
        fallback_start = Time.instant
        wall_token, wall_pos, emitted = append_exact_fallback_suffix!(
          weights, wall_state, wall_ids, wall_token, wall_pos, mtp_chain_tokens,
          mtp_spec_wall_gpu_fallback_chain, mtp_spec_wall_gpu_fallback_chain_min)
        wall_fallback_tokens += emitted
        wall_fallback_ms += elapsed_ms(fallback_start)
        wall_entry_skips += 1
        wall_entry_skip_tokens += emitted
        next
      end

      if mtp_spec_wall_exact_first && wall_ids.size < mtp_chain_tokens
        consumed_hidden = wall_hidden
        consumed_token = wall_token
        consumed_pos = wall_pos
        exact_first_start = Time.instant
        exact_hidden = ML::GGUF::Qwen35CPU.forward_hidden(weights, consumed_token, consumed_pos, wall_state)
        exact_next, _exact_logit = ML::GGUF::Qwen35CPU.hidden_top1(weights, exact_hidden)
        wall_exact_first_ms += elapsed_ms(exact_first_start)
        wall_exact_first_tokens += 1
        if mtp_state = wall_mtp_state
          # Invariant: persistent MTP KV contains tokens consumed before the
          # current boundary token. Exact-first consumes `consumed_token`; the
          # next lazy draft call will append `exact_next` exactly once.
          wall_mtp_ms += mtp_sync_state_token!(weights, mtp, consumed_hidden, consumed_token, consumed_pos, mtp_state)
        end
        wall_ids << exact_next
        wall_hidden = exact_hidden
      ...
      candidate_hiddens = [] of Array(Float32)
      draft_topk = ((mtp_spec_wall_top2_accounting && !mtp_spec_wall_top2_on_reject) || mtp_spec_wall_min_margin) ? 2 : 1
      draft_hidden = wall_hidden
      draft_token = wall_token
      draft_pos = wall_pos
      stateless_mtp_draft = mtp_draft_state_off || ENV["QWEN35_MTP_DRAFT_STATE_OFF"]? == "1"
      draft_state = if mtp_state = wall_mtp_state
                      stateless_mtp_draft ? nil : mtp_state
                    else
                      (!stateless_mtp_draft && draft_steps > 1) ? ML::GGUF::Qwen35MTP::State.new(draft_steps, weights.hparams.head_dim * weights.hparams.n_head_kv) : nil
                    end
      draft_generated = 0

      unless mtp_spec_wall_lazy_draft
        draft_steps.times do
          mtp_hidden, mtp_topk, step_ms = mtp_hidden_topk(weights, mtp, draft_hidden, draft_token, draft_pos, draft_topk, false, draft_state)
          ...
          candidate_hiddens << mtp_hidden
          wall_top2_promotions += 1 if promoted
          wall_draft_tokens += 1
          wall_mtp_ms += step_ms
          draft_hidden = if alpha = mtp_spec_wall_raw_blend
                           blend_hidden(draft_hidden, mtp_hidden, alpha)
                         else
                           mtp_hidden
                         end
      ...
      while !pass_rejected && !pass_finished
        ...
        stage_mtp_state_base_len = draft_state.try(&.length)
        if mtp_spec_wall_lazy_draft
          ...
          current_stage.times do
            ...
            if mtp_topk.size > 1
              ...
              pass_mtp_min_margin = pass_mtp_min_margin ? Math.min(pass_mtp_min_margin.not_nil!, margin) : margin
              if threshold = mtp_spec_wall_min_margin
                if stage_candidates.empty? && margin < threshold
            ...
            stage_candidate_hiddens << mtp_hidden
            wall_top2_promotions += 1 if promoted
            wall_draft_tokens += 1
            wall_mtp_ms += step_ms
            draft_hidden = if alpha = mtp_spec_wall_raw_blend
          ...
          stage_candidate_hiddens = candidate_hiddens[candidate_offset, current_stage]
          final_stage = candidate_offset + current_stage >= candidates.size
        end
        if margin_skip_stage
          if mtp_state = wall_mtp_state
            # A margin probe already consumed `stage_token` in the MTP cache.
            # Preserve that row if we continue after a one-token exact fallback.
            base_len = stage_mtp_state_base_len || mtp_state.length
            mtp_state.truncate!(base_len + 1)
          end
          fallback_start = Time.instant
          if mtp_spec_wall_min_margin_offramp
            wall_token, wall_pos, emitted = append_exact_fallback_suffix!(
              weights, wall_state, wall_ids, wall_token, wall_pos, mtp_chain_tokens,
              mtp_spec_wall_gpu_fallback_chain, mtp_spec_wall_gpu_fallback_chain_min)
            wall_fallback_tokens += emitted
          else
            exact_hidden = ML::GGUF::Qwen35CPU.forward_hidden(weights, stage_token, stage_pos, wall_state)
            exact_next, _exact_logit = ML::GGUF::Qwen35CPU.hidden_top1(weights, exact_hidden)
            wall_ids << exact_next if wall_ids.size < mtp_chain_tokens
            wall_hidden = exact_hidden
            wall_token = exact_next
            wall_pos = stage_pos + 1
        ...
        state_slot_probe_can_skip_backup = mtp_spec_wall_state_slot_probe &&
                                           !use_rec_rollback_log &&
                                           !mtp_spec_wall_top2_rescue_continue &&
                                           current_stage == 2 &&
                                           verify_tokens.size == 2 &&
                                           !checkpoint_index.nil?
        backup_needed = !use_rec_rollback_log && !state_slot_probe_can_skip_backup
        wall_state_slot_skipped_backups += 1 if state_slot_probe_can_skip_backup
        if backup_needed
        ...
        verified = begin
          ...
          elsif checkpoint_index
            checkpoint_state = ML::GGUF::Qwen35CPU::State.new(weights.hparams, max_seq)
            result = if use_rec_rollback_log
                       ML::GGUF::Qwen35CPU.prefill_tokens_hidden_top1s_recurrent_rollback_log(
                         weights,
                         verify_tokens,
                         stage_pos,
                         wall_state,
        ...
        if accepted_stage == stage_candidates.size
          ...
          if need_bonus && wall_ids.size < mtp_chain_tokens
            ...
            wall_token = stage_token
            wall_pos = stage_pos
            if mtp_spec_wall_resync_draft_hidden && mtp_spec_wall_lazy_draft
              draft_hidden = stage_hidden
              draft_token = stage_token
              draft_pos = stage_pos
            end
          end
          ...
          rejected_tail_tokens = Math.max(verified_token_count - accepted_stage - 1, 0)
          wall_rejected_tail_tokens += rejected_tail_tokens
          ...
          if mtp_spec_wall_top2_accounting
            ...
            if second_candidate >= 0
              ...
              if second_candidate == correction
                ...
                wall_top2_rescues += 1
                wall_top2_wrong_tail_tokens += rejected_tail_tokens
              end
            end
          end
          ...
          if verified_token_count == accepted_stage + 1
            ...
            wall_state_slot_recoveries += 1 if state_slot_probe_can_skip_backup
            row_base = accepted_stage * weights.hparams.n_embd
            wall_hidden = hidden_rows[row_base, weights.hparams.n_embd]
          elsif accepted_stage == 0 && checkpoint_state
            # The verifier ran into one wrong-tail row. Either restore the full
            # recurrent checkpoint, or roll back the logged DeltaNet update and
            # restore only the small conv checkpoint.
            wall_state_slot_recoveries += 1 if state_slot_probe_can_skip_backup
            rec_restore_start = Time.instant
            if use_rec_rollback_log
              ML::GGUF::Qwen35CPU.copy_recurrent_conv_metal_used!(
                wall_state,
                checkpoint_state.not_nil!,
                weights.hparams)
              ML::GGUF::Qwen35CPU.rollback_recurrent_ssm_metal_from_log!(
                wall_state,
                checkpoint_state.not_nil!,
                weights.hparams)
            else
            ...
            raise "state-slot probe cannot recover reject shape accepted_stage=#{accepted_stage} verified_token_count=#{verified_token_count}" if state_slot_probe_can_skip_backup
          ...
          correction_pos = stage_pos + accepted_stage + 1
          top2_miss_fallback = mtp_spec_wall_top2_miss_offramp &&
                               !top2_rescued_rejection &&
                               wall_ids.size + 1 < mtp_chain_tokens
          reject_offramp_fallback = !top2_miss_fallback &&
                                    mtp_spec_wall_reject_offramp > 0 &&
                                    consecutive_rejections >= mtp_spec_wall_reject_offramp &&
                                    wall_ids.size + 1 < mtp_chain_tokens
          if mtp_state = wall_mtp_state
            base_len = stage_mtp_state_base_len || mtp_state.length
            # The draft stage consumed `stage_token` plus each accepted
            # candidate before the correction boundary. The correction itself
            # becomes the next boundary token and will be appended by the next
            # MTP proposal, not by reject repair.
            mtp_state.truncate!(base_len + accepted_stage + 1)
          end

          wall_token = correction
          wall_pos = correction_pos
          wall_ids << correction if wall_ids.size < mtp_chain_tokens

          if top2_rescued_rejection && mtp_spec_wall_top2_rescue_continue
            consecutive_rejections = 0
          elsif top2_miss_fallback
      ...
      router_records << MtpSpecWallRouterPass.new(
        ...
        wall_verifier_tokens - pass_verifier_tokens_before,
        wall_rejected_tail_tokens - pass_rejected_tail_tokens_before,
    ...
    agg.rejected_tail_tokens += wall_rejected_tail_tokens
    agg.replay_tokens += wall_replay_tokens
    agg.fallback_tokens += wall_fallback_tokens
    agg.snapshot_tokens += wall_snapshot_tokens
    agg.state_slot_skipped_backups += wall_state_slot_skipped_backups
    agg.state_slot_recoveries += wall_state_slot_recoveries
    ...
    agg.margin_skips += wall_margin_skips
    agg.entry_skips += wall_entry_skips
    agg.entry_skip_tokens += wall_entry_skip_tokens
