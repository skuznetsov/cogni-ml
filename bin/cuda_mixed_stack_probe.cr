...
require "../src/ml/cuda/qwen_state_artifact_restorer"
...
def write_host_snapshot_file(snapshot : ML::CUDA::QwenMixedStackRunner::HostDecodeStateSnapshot, path : String) : Nil
  File.open(path, "w") do |file|
    snapshot.buffers.each { |buffer| file.write(buffer) }
  end
end

def sha256_file(path : String) : String
  output = IO::Memory.new
  status = Process.run("sha256sum", [path], output: output, error: Process::Redirect::Close)
  unless status.success?
    output.clear
    status = Process.run("shasum", ["-a", "256", path], output: output, error: Process::Redirect::Close)
  end
  raise "sha256 tool failed" unless status.success?

  output.to_s.split.first
end

def read_host_snapshot_file(path : String, template : ML::CUDA::QwenMixedStackRunner::HostDecodeStateSnapshot) : ML::CUDA::QwenMixedStackRunner::HostDecodeStateSnapshot
  buffers = template.buffers.map { |buffer| Bytes.new(buffer.size) }
  File.open(path, "r") do |file|
    buffers.each { |buffer| file.read_fully(buffer) }
  end
  ML::CUDA::QwenMixedStackRunner::HostDecodeStateSnapshot.new(buffers, template.buffer_roles,
    template.include_kv, template.kv_tokens, template.recurrent_bytes_total, template.kv_bytes_total)
end
...
def qwen_snapshot_from_host_decode_snapshot(snapshot : ML::CUDA::QwenMixedStackRunner::HostDecodeStateSnapshot,
  ...
  layer_ids.zip(runners).each do |layer_id, runner|
    ...
    in ML::CUDA::QwenRecurrentLayerRunner
      ...
      records << ML::GGUF::Qwen35StateSnapshot::Record.new(layer_id, ML::GGUF::Qwen35StateSnapshot::RecordKind::ConvState, conv, ML::StorageMode::Shared)
      records << ML::GGUF::Qwen35StateSnapshot::Record.new(layer_id, ML::GGUF::Qwen35StateSnapshot::RecordKind::SsmState, ssm, ML::StorageMode::Shared)
      idx += 2
    in ML::CUDA::QwenFullAttnLayerRunner
      if snapshot.include_kv
        k_cache = snapshot.buffers[idx]
        v_cache = snapshot.buffers[idx + 1]
        records << ML::GGUF::Qwen35StateSnapshot::Record.new(layer_id, ML::GGUF::Qwen35StateSnapshot::RecordKind::KCache, k_cache, ML::StorageMode::Shared)
        records << ML::GGUF::Qwen35StateSnapshot::Record.new(layer_id, ML::GGUF::Qwen35StateSnapshot::RecordKind::VCache, v_cache, ML::StorageMode::Shared)
        idx += 2
      end
    end
  end
...
class RecurrentInt8CodecBench
  struct Segment
    getter buffer_index : Int32
    getter q_offset : Int32
    getter scale_offset : Int32
    getter values : Int32
    getter blocks : Int32

    def initialize(@buffer_index : Int32,
                   @q_offset : Int32,
                   @scale_offset : Int32,
                   @values : Int32,
                   @blocks : Int32)
  ...
  getter quant : Array(Int8)?
  getter scales : Array(Float32)?
  ...
  def initialize(@raw_bytes : UInt64,
                 ...
                 @quant : Array(Int8)? = nil,
                 @scales : Array(Float32)? = nil,
...
class CudaRecurrentInt8CodecRestorer
  ...
  def initialize(codec : RecurrentInt8CodecBench,
    ...
    @module = ML::CUDA::CUDAModule.load(PTX, "recurrent_int8_codec_probe")
    @fn = @module.function("recurrent_block_i8_decode_f32")
    @q_device = ML::CUDA::DeviceBuffer.new(@quant.size.to_u64)
    @scales_device = ML::CUDA::DeviceBuffer.new((@scales.size * sizeof(Float32)).to_u64)
    @q_ptr = Pointer(ML::CUDA::DevicePtr).malloc(1)
    @scales_ptr = Pointer(ML::CUDA::DevicePtr).malloc(1)
    @out_ptr = Pointer(ML::CUDA::DevicePtr).malloc(1)
    @values_ptr = Pointer(UInt32).malloc(1)
    @block_size_ptr = Pointer(UInt32).malloc(1)
    @params = Pointer(Void*).malloc(5)
    @params[0] = @q_ptr.as(Void*)
    @params[1] = @scales_ptr.as(Void*)
    @params[2] = @out_ptr.as(Void*)
    @params[3] = @values_ptr.as(Void*)
    @params[4] = @block_size_ptr.as(Void*)
    @block_size_ptr.value = @block_size.to_u32
    @closed = false
  end
  ...
  def restore(stack : ML::CUDA::QwenMixedStackRunner,
    ...
    t_h2d = Time.instant
    ML::CUDA.copy_htod!(@q_device.ptr, @quant.to_unsafe.as(Void*), @quant.size.to_u64, "recurrent codec q")
    ML::CUDA.copy_htod!(@scales_device.ptr, @scales.to_unsafe.as(Void*), (@scales.size * sizeof(Float32)).to_u64, "recurrent codec scales")
    ...
    raise "unused recurrent codec segments" unless segment_idx == @segments.size
    ML::CUDA.synchronize!("cuCtxSynchronize(recurrent codec gpu decode)")
  ...
  private def decode_segment(segment : RecurrentInt8CodecBench::Segment, out_ptr : ML::CUDA::DevicePtr) : Nil
    @q_ptr.value = @q_device.ptr + segment.q_offset.to_u64
    @scales_ptr.value = @scales_device.ptr + (segment.scale_offset * sizeof(Float32)).to_u64
  ...
  private def restore_kv(stack : ML::CUDA::QwenMixedStackRunner,
    ...
    stack.runners.each do |runner|
      ...
      in ML::CUDA::QwenFullAttnLayerRunner
        ...
        ML::CUDA.copy_htod!(runner.k_cache_device_ptr, k_cache.to_unsafe.as(Void*), kv_bytesize, "restore codec host k_cache")
        ML::CUDA.copy_htod!(runner.v_cache_device_ptr, v_cache.to_unsafe.as(Void*), kv_bytesize, "restore codec host v_cache")
        idx += 2
      end
    end
    raise "unused host decode snapshot buffers" unless idx == snapshot.buffers.size
...
def recurrent_int8_codec_bench(snapshot : ML::CUDA::QwenMixedStackRunner::HostDecodeStateSnapshot,
                               ...
                               build_encoded_snapshot : Bool = false) : RecurrentInt8CodecBench
  ...
  snapshot.buffers.each_with_index do |buffer, idx|
    ...
    count = buffer.size // 4
    segment_q_offset = q_idx
    segment_scale_offset = scales.size
    offset = 0
    while offset < count
      n = Math.min(block_size, count - offset)
      max_abs = 0.0_f32
      n.times do |i|
        v_abs = values[offset + i].abs
        max_abs = v_abs if v_abs > max_abs
      end
      max_abs_global = max_abs if max_abs > max_abs_global
      scale = max_abs > 0.0_f32 ? max_abs / 127.0_f32 : 1.0_f32
      scales << scale
      n.times do |i|
        q = (values[offset + i] / scale).round.to_i
        quant[q_idx + i] = q.clamp(-127, 127).to_i8
      end
      q_idx += n
      offset += n
    end
    segments << RecurrentInt8CodecBench::Segment.new(idx, segment_q_offset, segment_scale_offset,
      count, scales.size - segment_scale_offset)
  end
...
known_replay_trusted_artifact_codec_block_provided = false
...
OptionParser.parse do |p|
  ...
  p.on("--known-replay-trusted-artifact-codec-block N", "Block size for recurrent block-INT8 codec bench, default 256; late-format=int8 defaults to 8 unless overridden") { |v| known_replay_trusted_artifact_codec_block = v.to_i; known_replay_trusted_artifact_codec_block_provided = true }
...
if known_replay_trusted_artifact_codec_late_format == "int8" && !known_replay_trusted_artifact_codec_block_provided
...
raise "--known-replay-trusted-artifact-encoded-restorer requires GPU decode restore checks" if known_replay_trusted_artifact_encoded_restorer && !known_replay_trusted_artifact_codec_gpu_decode_check
...
known_replay_schedule_report.each do |size|
  raise "--known-replay-schedule-report sizes must be positive" unless size > 0
end
raise "--known-replay-schedule-report requires --known-replay-candidates or --known-replay-history" if !known_replay_schedule_report.empty? && known_replay_candidates.empty?
...
begin
  ...
  known_replay_trusted_artifact_recurrent_bytes = 0_u64
  known_replay_trusted_artifact_kv_bytes = 0_u64
  known_replay_trusted_artifact_write_ms = 0.0
  known_replay_trusted_artifact_read_ms = 0.0
  known_replay_trusted_artifact_hash_ms = 0.0
  known_replay_trusted_artifact_sha256_12 = ""
  known_replay_trusted_artifact_codec_raw_bytes = 0_u64
  known_replay_trusted_artifact_codec_encoded_bytes = 0_u64
  known_replay_trusted_artifact_codec_blocks = 0
  known_replay_trusted_artifact_codec_encode_ms = 0.0
  ...
  known_replay_trusted_artifact_codec_continuation_inputs = [] of Int32
  known_replay_trusted_artifact_codec_continuation_expected = [] of Int32
  known_replay_trusted_artifact_codec_continuation_exact_top1 = [] of Int32
  ...
  known_replay_trusted_artifact_codec_gpu_decode_check_ok = false
  known_replay_trusted_artifact_codec_restore_policy = "none"
    ...
    if known_replay_trusted_artifact_restore
      ...
      if known_replay_trusted_artifact_host_restore
            ...
            if codec_policy_gpu_decode
              if known_replay_trusted_artifact_encoded_restorer
                ...
                codec_gpu_restorer = CudaRecurrentInt8CodecRestorer.new(codec,
                  ...
                  known_replay_trusted_artifact_codec_raw_recurrent_layers)
              end
            end
          end
        end
        ...
        if known_replay_trusted_artifact_io_probe
          ...
          begin
            t_artifact_write = Time.instant
            write_host_snapshot_file(artifact_snapshot, artifact_path)
        ...
        if known_replay_trusted_artifact_codec_restore_check
          ...
          encoded_restorer = codec_encoded_restorer
          if codec_policy_gpu_decode
            if encoded_restorer
              # v2 .qkv encoded artifact restorer covers both BF16 and block-INT8.
            elsif codec_policy_format == "bf16"
          ...
          raise "known-replay history too short for codec continuation inputs" unless continuation_input_index + known_replay_trusted_artifact_codec_restore_steps <= known_replay_history.size

          known_replay_trusted_artifact_codec_continuation_inputs =
            known_replay_history[continuation_input_index, known_replay_trusted_artifact_codec_restore_steps]
          known_replay_trusted_artifact_codec_continuation_expected =
            Array(Int32).new(known_replay_trusted_artifact_codec_restore_steps) do |step|
              expected_idx = continuation_expected_index + step
              expected_idx < known_replay_history.size ? known_replay_history[expected_idx] : -1
            end

          known_replay_trusted_artifact_codec_continuation_inputs.each_with_index do |input_id, step|
            ...
            exact_next = run_trusted_artifact_continuation(mixed_stack, token_embd,
              input_id, hidden, tokens, continuation_position)
            known_replay_trusted_artifact_codec_continuation_exact_top1 << exact_next[:top1]
          ...
          if codec_policy_raw_host
            ...
            if encoded_restorer
              encoded_restorer.not_nil!.restore(mixed_stack)
              known_replay_trusted_artifact_codec_gpu_decode_h2d_ms = encoded_restorer.not_nil!.h2d_ms
              known_replay_trusted_artifact_codec_gpu_decode_kernel_ms = encoded_restorer.not_nil!.kernel_ms
              known_replay_trusted_artifact_codec_gpu_decode_kv_ms = encoded_restorer.not_nil!.kv_ms
              known_replay_trusted_artifact_codec_raw_recurrent_ms = encoded_restorer.not_nil!.raw_recurrent_ms
              known_replay_trusted_artifact_codec_gpu_decode_restore_ms = encoded_restorer.not_nil!.restore_ms
            elsif codec_policy_format == "bf16"
          ...
          known_replay_trusted_artifact_codec_continuation_inputs.each_with_index do |input_id, step|
            ...
            decoded_next = run_trusted_artifact_continuation(mixed_stack, token_embd,
              input_id, hidden, tokens, continuation_position)
            known_replay_trusted_artifact_codec_continuation_decoded_top1 << decoded_next[:top1]
            known_replay_trusted_artifact_codec_decoded_next_ms += decoded_next[:ms]
          end

          known_replay_trusted_artifact_codec_next_input = known_replay_trusted_artifact_codec_continuation_inputs[0]
          known_replay_trusted_artifact_codec_next_expected = known_replay_trusted_artifact_codec_continuation_expected[0]
          known_replay_trusted_artifact_codec_next_exact_top1 = known_replay_trusted_artifact_codec_continuation_exact_top1[0]
          known_replay_trusted_artifact_codec_next_decoded_top1 = known_replay_trusted_artifact_codec_continuation_decoded_top1[0]
          known_replay_trusted_artifact_codec_continuation_exact_top1.each_with_index do |exact_id, step|
            decoded_id = known_replay_trusted_artifact_codec_continuation_decoded_top1[step]
            expected_id = known_replay_trusted_artifact_codec_continuation_expected[step]
            known_replay_trusted_artifact_codec_continuation_parity_count += 1 if decoded_id == exact_id
            known_replay_trusted_artifact_codec_continuation_source_count += 1 if expected_id >= 0 && decoded_id == expected_id
          end
          known_replay_trusted_artifact_codec_next_parity_ok =
            known_replay_trusted_artifact_codec_continuation_parity_count == known_replay_trusted_artifact_codec_restore_steps
          ...
          if known_replay_trusted_artifact_codec_free_run_steps > 0
            ...
            if codec_policy_raw_host
              ...
              if encoded_restorer
                ...
                known_replay_trusted_artifact_codec_raw_recurrent_ms = encoded_restorer.not_nil!.raw_recurrent_ms
                known_replay_trusted_artifact_codec_gpu_decode_restore_ms = encoded_restorer.not_nil!.restore_ms
                known_replay_trusted_artifact_codec_restore_ms = encoded_restorer.not_nil!.restore_ms
              elsif codec_policy_format == "bf16"
                gpu_bf16_restorer.not_nil!.restore(mixed_stack, restore_snapshot)
                known_replay_trusted_artifact_codec_gpu_decode_h2d_ms = gpu_bf16_restorer.not_nil!.h2d_ms
                known_replay_trusted_artifact_codec_gpu_decode_kernel_ms = gpu_bf16_restorer.not_nil!.kernel_ms
          ...
          known_replay_trusted_artifact_codec_gpu_decode_check_ok =
            !known_replay_trusted_artifact_codec_gpu_decode_check ||
              (known_replay_trusted_artifact_codec_next_parity_ok &&
        ...
        known_replay_trusted_artifact_snapshot_ms = (Time.instant - t_artifact_snapshot).total_milliseconds
        known_replay_trusted_artifact_bytes = artifact_snapshot.buffers.sum(0_u64) { |buffer| buffer.bytesize.to_u64 }
        begin
          poison_xs = Array(Float32).new(tokens * hidden, 0.0_f32)
          emb = ML::GGUF::Qwen35CPU.embedding_lookup(token_embd, known_replay_history[0])
          hidden.times { |h| poison_xs[h] = emb[h] }
  ...
  ok &&= known_replay_trusted_artifact_codec_gpu_decode_check_ok if known_replay_trusted_artifact_codec_gpu_decode_check
  if greedy_loop_tokens > 0
    if perf_only
  ...
  unless known_replay_candidates.empty?
    ...
    if known_replay_trusted_artifact_restore
      ...
      lines << "known_replay_trusted_artifact_recurrent_bytes=#{known_replay_trusted_artifact_recurrent_bytes}"
      lines << "known_replay_trusted_artifact_kv_bytes=#{known_replay_trusted_artifact_kv_bytes}"
      ...
      lines << "known_replay_trusted_artifact_codec_ratio=#{known_replay_trusted_artifact_codec_raw_bytes > 0 ? (known_replay_trusted_artifact_codec_encoded_bytes.to_f64 / known_replay_trusted_artifact_codec_raw_bytes).round(4) : 0.0}"
      lines << "known_replay_trusted_artifact_codec_blocks=#{known_replay_trusted_artifact_codec_blocks}"
      lines << "known_replay_trusted_artifact_codec_encode_ms=#{known_replay_trusted_artifact_codec_encode_ms.round(3)}"
      ...
      lines << "known_replay_trusted_artifact_codec_restore_steps=#{known_replay_trusted_artifact_codec_restore_steps}"
      ...
      lines << "known_replay_trusted_artifact_codec_continuation_inputs=#{known_replay_trusted_artifact_codec_continuation_inputs.join(",")}"
      lines << "known_replay_trusted_artifact_codec_continuation_expected=#{known_replay_trusted_artifact_codec_continuation_expected.join(",")}"
      lines << "known_replay_trusted_artifact_codec_continuation_exact_top1=#{known_replay_trusted_artifact_codec_continuation_exact_top1.join(",")}"
      ...
      lines << "known_replay_trusted_artifact_codec_restore_policy=#{known_replay_trusted_artifact_codec_restore_policy}"
