...
OptionParser.parse(ARGV) do |parser|
  ...
  parser.on("--perf-only", "Run only aligned base0 P1024/P2048 timing controls") { run_perf = true; perf_only = true }
...
puts "device=#{ML::Metal::Device.instance.name.inspect} baseline=qwen35_attn_decode_rows_sg4 flash_source=#{flash_source_path || "embedded"}"
  ...
  CORRECTNESS_CASES.each do |shape|
    ...
    gqa_groups.each do |heads_per_group|
      ...
      run_baseline(baseline_pipe, q_buf, gate_buf, k_buf, v_buf, baseline_out,
        shape, dispatch_tokens, inputs.n_head, inputs.n_head_kv, heads_per_group)
...
if run_perf
  perf_cases = if perf_only
                 [ShapeCase.new(0, 1024), ShapeCase.new(0, 2048)]
               else
                 [
  ...
  perf_cases.each do |shape|
    ...
    warmup.times do
      run_baseline(baseline_pipe, q_buf, gate_buf, k_buf, v_buf, baseline_out,
        shape, shape.n_tokens, inputs.n_head, inputs.n_head_kv, heads_per_group)
      run_flash(flash_pipe, q_buf, gate_buf, k_buf, v_buf, flash_out,
        shape, inputs.n_head, inputs.n_head_kv, heads_per_group)
      run_flash(flash_pipe, q_buf, gate_buf, k_buf, v_buf, flash_out,
        shape, inputs.n_head, inputs.n_head_kv, heads_per_group)
