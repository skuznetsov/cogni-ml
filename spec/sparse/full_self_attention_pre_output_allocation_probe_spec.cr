{% unless flag?(:trellis2_sparse_full_self_attention_pre_output_allocation_probe) %}
  {% skip_file %}
{% end %}

{% unless flag?(:darwin) && flag?(:cpu_only) %}
  {% raise "sparse full self-attention pre-output allocation probe requires Darwin CPU-only execution" %}
{% end %}
...
private def measured_sparse_full_attention_pre_output_bytes(& : ->) : Int64
  GC.collect
...
describe "TRELLIS.2 sparse full self-attention pre-output allocation" do
  it "allocates only the composed leaf payloads and one score row" do
    ...
    linear.weight.data.cpu_data.not_nil!.fill(0.0_f32)
    linear.bias.not_nil!.data.cpu_data.not_nil!.fill(0.0_f32)
    linear.weight.requires_grad = false
    linear.bias.not_nil!.requires_grad = false
    q_gamma = ML::Tensor.ones(1, channels)
    k_gamma = ML::Tensor.ones(1, channels)

    output = uninitialized ML::Sparse::TensorCPU
    allocated = measured_sparse_full_attention_pre_output_bytes do
    ...
    composed_payload_bytes =
      3_i64 * packed_payload_bytes + output_payload_bytes + score_row_bytes
    ...
    measurement = JSON.build do |json|
      json.object do
        json.field "probe", "trellis2-sparse-full-self-attention-pre-output-v1"
