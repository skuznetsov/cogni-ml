...
describe ML::GGUF::DiffusionGemmaCPU do
  ...
  it "gates prompt materialization row batching by prompt length env policy" do
    old_policy = ENV["DIFFUSION_GEMMA_PROMPT_CACHE_POLICY"]?
    old_policy_off = ENV["DIFFUSION_GEMMA_PROMPT_CACHE_POLICY_OFF"]?
    ...
    old_min = ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MIN_PROMPT"]?
    old_max = ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MAX_PROMPT"]?
    begin
      ENV.delete("DIFFUSION_GEMMA_PROMPT_CACHE_POLICY")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_CACHE_POLICY_OFF")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS_OFF")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MIN_PROMPT")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MAX_PROMPT")
      ...
      ML::GGUF::DiffusionGemmaCPU.prompt_materialize_batch_rows_enabled?(2).should be_true

      ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MIN_PROMPT"] = "4"
      ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MAX_PROMPT"] = "8"
      ML::GGUF::DiffusionGemmaCPU.prompt_materialize_batch_rows_enabled?(2).should be_false
      ML::GGUF::DiffusionGemmaCPU.prompt_materialize_batch_rows_enabled?(4).should be_true
      ML::GGUF::DiffusionGemmaCPU.prompt_materialize_batch_rows_enabled?(8).should be_true
      ML::GGUF::DiffusionGemmaCPU.prompt_materialize_batch_rows_enabled?(16).should be_false

      ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS_OFF"] = "1"
      ML::GGUF::DiffusionGemmaCPU.prompt_materialize_batch_rows_enabled?(8).should be_false
    ensure
      if old_policy
        ENV["DIFFUSION_GEMMA_PROMPT_CACHE_POLICY"] = old_policy
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_CACHE_POLICY")
      end
      if old_policy_off
        ENV["DIFFUSION_GEMMA_PROMPT_CACHE_POLICY_OFF"] = old_policy_off
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_CACHE_POLICY_OFF")
      end
      if old_enabled
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS"] = old_enabled
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS")
      end
      if old_off
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS_OFF"] = old_off
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS_OFF")
      end
      if old_min
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MIN_PROMPT"] = old_min
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MIN_PROMPT")
      end
      if old_max
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MAX_PROMPT"] = old_max
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MAX_PROMPT")
      end
    end
  ...
  it "bundles prompt-cache policy without enabling c8 decode routes" do
    ...
    begin
      ...
      ML::GGUF::DiffusionGemmaCPU.prompt_cache_policy_requested?.should be_true
      ML::GGUF::DiffusionGemmaCPU.prompt_materialize_batch_rows_enabled?(1).should be_false
  ...
  it "can route grouped MoE expert batches through the resident graph path" do
    ...
    old_gpu_gather = ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_GATHER"]?
    ...
    old_down_command = ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND"]?
    ...
    old_gpu_prenorm_max = ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_MAX_CANVAS"]?
    begin
      w = ML::GGUF::DiffusionGemmaWeights.from_gguf(DIFFUSION_GEMMA_26B_Q4KM)
      hp = w.hparams
      rows_by_id = (0...4).map { |token_id| ML::GGUF::DiffusionGemmaCPU.zero_sc_canvas_embedding(w, token_id) }
      ...
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND"] = "1"
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND_MIN_CANVAS"] = "4"
      down_command = ML::GGUF::DiffusionGemmaCPU.moe_ffn_grouped_expert_rows(w, 0, rows, rows_by_id.size, routes_by_row)
      down_command.size.should eq(expected.size)
      max_diff = 0.0_f32
      expected.size.times do |i|
        diff = (expected[i] - down_command[i]).abs
        max_diff = diff if diff > max_diff
      end
      max_diff.should be < 1.0e-4_f32
      ...
      max_diff = 0.0_f32
      actual.size.times do |i|
        diff = (actual[i] - gpu_gather[i]).abs
        max_diff = diff if diff > max_diff
      end
      max_diff.should be < 1.0e-3_f32

      ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM"] = "1"
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_MIN_CANVAS"] = "4"
      gpu_prenorm = ML::GGUF::DiffusionGemmaCPU.moe_ffn_grouped_expert_rows(w, 0, rows, rows_by_id.size, routes_by_row)
      gpu_prenorm.size.should eq(gpu_gather.size)
      max_diff = 0.0_f32
      gpu_gather.size.times do |i|
        diff = (gpu_gather[i] - gpu_prenorm[i]).abs
        max_diff = diff if diff > max_diff
      end
      max_diff.should be < 1.0e-3_f32
      ...
      ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM")
      ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_MIN_CANVAS")
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_gpu_prenorm_enabled?(8).should be_false
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM"] = "1"
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_gpu_prenorm_enabled?(4).should be_false
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_gpu_prenorm_enabled?(8).should be_true
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_MIN_CANVAS"] = "8"
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_MAX_CANVAS"] = "8"
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_gpu_prenorm_enabled?(4).should be_false
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_gpu_prenorm_enabled?(8).should be_true
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_gpu_prenorm_enabled?(16).should be_false
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_OFF"] = "1"
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_gpu_prenorm_enabled?(8).should be_false
      ...
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND"] = "1"
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_resident_down_command_enabled?(4).should be_false
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_resident_down_command_enabled?(8).should be_true
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND_MIN_CANVAS"] = "4"
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND_MAX_CANVAS"] = "8"
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_resident_down_command_enabled?(4).should be_true
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_resident_down_command_enabled?(16).should be_false
      ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_GRAPH"] = "1"
      ML::GGUF::DiffusionGemmaCPU.moe_grouped_resident_down_command_enabled?(8).should be_false
      ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_GRAPH")
      ...
      if old_gpu_gather
        ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_GATHER"] = old_gpu_gather
      else
        ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_GPU_GATHER")
      end
      if old_gpu_gather_off
        ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_GATHER_OFF"] = old_gpu_gather_off
      else
        ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_GPU_GATHER_OFF")
      end
      if old_gpu_gather_min
        ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_GATHER_MIN_CANVAS"] = old_gpu_gather_min
      else
        ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_GPU_GATHER_MIN_CANVAS")
      end
      if old_gpu_gather_max
      ...
      if old_down_command
        ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND"] = old_down_command
      else
        ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND")
      end
      if old_down_command_off
        ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND_OFF"] = old_down_command_off
      else
        ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND_OFF")
      end
      if old_down_command_min
        ENV["DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND_MIN_CANVAS"] = old_down_command_min
      else
        ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_RESIDENT_DOWN_COMMAND_MIN_CANVAS")
      end
      if old_down_command_max
      ...
      if old_gpu_prenorm_off
        ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_OFF"] = old_gpu_prenorm_off
      else
        ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_OFF")
      end
      if old_gpu_prenorm_min
        ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_MIN_CANVAS"] = old_gpu_prenorm_min
      else
        ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_MIN_CANVAS")
      end
      if old_gpu_prenorm_max
        ENV["DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_MAX_CANVAS"] = old_gpu_prenorm_max
      else
        ENV.delete("DIFFUSION_GEMMA_MOE_GROUPED_GPU_PRENORM_MAX_CANVAS")
      end
    end
  end
  ...
  it "builds a bounded prompt layer cache from prompt-only rows" do
    ...
    cache.materialize_ms_by_layer.size.should eq(1)
    cache.materialize_context_ms_by_layer.size.should eq(1)
    cache.materialize_attention_out_ms_by_layer.size.should eq(1)
    cache.materialize_shared_ffn_ms_by_layer.size.should eq(1)
    cache.materialize_moe_ffn_ms_by_layer.size.should eq(1)
    cache.materialize_combine_scale_ms_by_layer.size.should eq(1)
  ...
  it "keeps batched prompt materialization cache parity" do
    pending!("DiffusionGemma 26B GGUF not found") unless File.exists?(DIFFUSION_GEMMA_26B_Q4KM)

    w = ML::GGUF::DiffusionGemmaWeights.from_gguf(DIFFUSION_GEMMA_26B_Q4KM)
    il = 0
    prompt_rows_by_pos = [1, 2, 3, 4].map do |token_id|
      ML::GGUF::DiffusionGemmaCPU.scaled_embedding_lookup(w, token_id)
    end
    prompt_rows = [] of Float32
    prompt_rows_by_pos.each { |row| prompt_rows.concat(row) }
    mask = ML::GGUF::DiffusionGemmaAttentionMask.new(prompt_len: 4, canvas_len: 1, sliding_window: 3)
    prompt_routes = prompt_rows_by_pos.map do |row|
      ML::GGUF::DiffusionGemmaCPU.route_experts(w, il, row)[0, 1]
    end

    old_policy = ENV["DIFFUSION_GEMMA_PROMPT_CACHE_POLICY"]?
    old_policy_off = ENV["DIFFUSION_GEMMA_PROMPT_CACHE_POLICY_OFF"]?
    old_batch = ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS"]?
    old_batch_off = ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS_OFF"]?
    old_batch_min = ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MIN_PROMPT"]?
    old_batch_max = ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MAX_PROMPT"]?
    old_grouped = ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_GROUPED_MOE"]?
    old_grouped_off = ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_GROUPED_MOE_OFF"]?
    old_proj_metal = ENV["DIFFUSION_GEMMA_PROMPT_PROJ_METAL"]?
    old_proj_metal_off = ENV["DIFFUSION_GEMMA_PROMPT_PROJ_METAL_OFF"]?
    old_proj_metal_min = ENV["DIFFUSION_GEMMA_PROMPT_PROJ_METAL_MIN_BATCH"]?
    begin
      ENV.delete("DIFFUSION_GEMMA_PROMPT_CACHE_POLICY")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_CACHE_POLICY_OFF")
      ...
      ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MIN_PROMPT")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MAX_PROMPT")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_GROUPED_MOE")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_GROUPED_MOE_OFF")
      ENV.delete("DIFFUSION_GEMMA_PROMPT_PROJ_METAL")
      ENV["DIFFUSION_GEMMA_PROMPT_PROJ_METAL_OFF"] = "1"
      ENV.delete("DIFFUSION_GEMMA_PROMPT_PROJ_METAL_MIN_BATCH")
      ...
      batched.layers.should eq(base.layers)
      batched.projections_by_layer.size.should eq(base.projections_by_layer.size)
      batched.projections_by_layer[0].each_with_index do |projection, pos|
        base_projection = base.projections_by_layer[0][pos]
        projection.q.should eq(base_projection.q)
        projection.k.should eq(base_projection.k)
        projection.v.should eq(base_projection.v)
      end
      ...
      if old_batch
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS"] = old_batch
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS")
      end
      if old_batch_off
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS_OFF"] = old_batch_off
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_ROWS_OFF")
      end
      if old_batch_min
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MIN_PROMPT"] = old_batch_min
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MIN_PROMPT")
      end
      if old_batch_max
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MAX_PROMPT"] = old_batch_max
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_BATCH_MAX_PROMPT")
      end
      if old_grouped
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_GROUPED_MOE"] = old_grouped
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_GROUPED_MOE")
      end
      if old_grouped_off
        ENV["DIFFUSION_GEMMA_PROMPT_MATERIALIZE_GROUPED_MOE_OFF"] = old_grouped_off
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_MATERIALIZE_GROUPED_MOE_OFF")
      end
      if old_proj_metal
      ...
      if old_proj_metal_off
        ENV["DIFFUSION_GEMMA_PROMPT_PROJ_METAL_OFF"] = old_proj_metal_off
      else
        ENV.delete("DIFFUSION_GEMMA_PROMPT_PROJ_METAL_OFF")
      end
      if old_proj_metal_min
...
describe ML::GGUF::DiffusionGemmaWeights do
  it "maps local DiffusionGemma tensors structurally without dequantizing large weights" do
    ...
    swa.ffn_gate_up_expert_qws.not_nil!.size.should eq(128)
    swa.ffn_gate_expert_qws.not_nil!.size.should eq(128)
    swa.ffn_up_expert_qws.not_nil!.size.should eq(128)
    swa.ffn_down_expert_qws.size.should eq(128)
    swa.ffn_gate_up_expert_qws.not_nil![0].out_dim.should eq(1408)
    swa.ffn_gate_expert_qws.not_nil![0].out_dim.should eq(704)
    swa.ffn_up_expert_qws.not_nil![0].out_dim.should eq(704)
    swa.ffn_down_expert_qws[0].out_dim.should eq(2816)
    gate_up_row_bytes = ML::GGUF::QuantMatmul.row_bytes(swa.ffn_gate_up_exps_qw.not_nil!.type, 2816)
    down_row_bytes = ML::GGUF::QuantMatmul.row_bytes(swa.ffn_down_exps_qw.type, 704)
    swa.ffn_gate_expert_qws.not_nil![0].raw.to_unsafe.address.should eq(swa.ffn_gate_up_expert_qws.not_nil![0].raw.to_unsafe.address)
    swa.ffn_up_expert_qws.not_nil![0].raw.to_unsafe.address.should eq(swa.ffn_gate_up_expert_qws.not_nil![0].raw.to_unsafe.address + 704_u64 * gate_up_row_bytes.to_u64)
    swa.ffn_gate_up_expert_qws.not_nil![1].raw.to_unsafe.address.should eq(swa.ffn_gate_up_expert_qws.not_nil![0].raw.to_unsafe.address + 1408_u64 * gate_up_row_bytes.to_u64)
    swa.ffn_down_expert_qws[1].raw.to_unsafe.address.should eq(swa.ffn_down_expert_qws[0].raw.to_unsafe.address + 2816_u64 * down_row_bytes.to_u64)
