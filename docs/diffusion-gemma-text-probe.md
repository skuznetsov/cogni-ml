...
`mixed_fallback_residual` means accepted windows eliminated/replayed that phase
but exact fallback still pays it, while `residual_materialize_boundary` points
at a larger CPU-materialization or tail-fusion corridor. Phase rows include both
`unsafe_variant_ms` and `mixed_variant_ms`; use the mixed fields for certified
route decisions and the unsafe fields only as all-fast diagnostic bounds. For
exact fallback windows, the atlas also reads the child
