...
- Resident stdin smoke with `MAX_ROWS=2 RPI5_WARMUPS=3 scripts/rpi5_q6_resident_stdin_smoke.sh /tmp/qwen35_2b_allowed_head_capture_20260609_223005.jsonl 4` now sends hidden vectors as binary stdin frames, not per-request `.f32` files. It emitted `resident_stdin_result` rows with `top1_match=true`, `max_abs_diff<=2.86102e-06`, warmed request times `0.159ms/0.267ms`, and `throttled=0x0`; remote temp cleanup was empty. Boundary: this is a daemon-protocol proof using a copied batch source file in the smoke wrapper, not the final product IPC or Crystal runtime adapter.
