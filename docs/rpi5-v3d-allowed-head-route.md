...
- `bin/qwen35_generate.cr` can now install the resident transport explicitly with `QWEN35_RPI5_RESIDENT_COMMAND_JSON`, a JSON argv array such as `["ssh","raspberrypi.local","...remote resident command..."]`. The adapter does not shell-split env strings; any remote shell command is an explicit argv element passed to `ssh`. Request summaries include `rpi5_resident_transport=true/false`. Boundary: this only wires the process transport into the existing constrained `forward_top1_allowed` hook; it still requires a real resident worker command and parity smoke before promotion.
- Product-path resident parity smoke: with a real SSH/Vulkan `QWEN35_RPI5_RESIDENT_COMMAND_JSON`, Qwen3.5-2B structured `edit_mode(mode=safe,dry_run=true)` generated exactly the same token ids as the local control. Control markers were `allowed_head.cpu_selected_metal_hidden=28`, `allowed_head.cpu_threshold=28`, `allowed_head.metal_q6=2`; resident markers were `allowed_head.cpu_selected_metal_hidden=30`, `allowed_head.cpu_threshold=28`, `allowed_head.rpi5_resident=2`, proving the two finite value frontiers routed through the Pi worker. Boundary: decode was slower (`814.5ms` resident vs `301.0ms` control) because this is still SSH/stdin process transport, so this is parity/wiring evidence, not a speed claim.
...
5. Replace SSH/stdin with a lower-overhead local-network worker transport or persistent socket protocol, then rerun the same product-path parity smoke and measure route-local latency.
