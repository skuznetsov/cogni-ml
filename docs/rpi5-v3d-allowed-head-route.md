...
- Transport budget check with `RAW_OUTPUT=0 REPEATS=30 RPI5_WARMUPS=3 scripts/rpi5_q6_transport_budget.sh /tmp/qwen35_2b_allowed_head_capture_20260609_223005.jsonl 4` replayed the filtered `16` real-hidden rows with `gpu_ms=1.842`, `cpu_ms=3.221`, `speedup=1.749x`, and `max_abs_diff=8.58307e-06`, but the current remote probe envelope measured `remote_wall_ms=605.304`, including `prepack_load_ms=127.629` and estimated setup overhead `413.668ms` (`224.6x` one averaged GPU dispatch). The same run emitted `resident_budget_result` with `resident_request_ms=1.842`, `resident_request_ms_per_row=0.115125`, and `cpu_selected_ms_per_row=0.201313`. This is a falsifier for SSH/process-per-call product routing and a target for the resident adapter.
