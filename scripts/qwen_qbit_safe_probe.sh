#!/usr/bin/env bash
# Build and run the experimental QBit model probe under unified-memory guards.
set -euo pipefail
...
PROBE_BIN="${QWEN_QBIT_PROBE_BIN:-/private/tmp/qwen35_qbit_cache_probe_release}"
...
CRYSTAL_CACHE_DIR="${CRYSTAL_CACHE_DIR:-/private/tmp/cogni_ml_qbit_safe_probe_cache}"
...
if [[ ! -x "$PROBE_BIN" || "${QWEN_QBIT_REBUILD:-0}" == "1" ]]; then
  "$RUN_SAFE" "$CRYSTAL_BIN" "$BUILD_TIMEOUT_SEC" "$BUILD_MAX_TREE_MB" \
    build bin/qwen35_qbit_cache_probe.cr \
    -o "$PROBE_BIN" --release --no-debug --error-trace \
    --link-flags="$ROOT/build/bridge.o -framework Metal -framework Foundation -lc++"
fi

# Build and model execution are deliberately separate processes so compiler
# residency cannot overlap the GGUF mapping and Metal state allocations.
