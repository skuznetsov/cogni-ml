...
OptionParser.parse do |p|
  p.banner = "Usage: gemma4_profile_atlas.cr [profile.log ...] [--top=N] [--tsv]"
...
def ascii_profile_text(raw : String) : String
  String.build do |io|
...
def read_profile_text(path : String) : String
  ascii_profile_text(File.read(path))
end

text = if input_paths.empty?
         ascii_profile_text(STDIN.gets_to_end)
       else
         input_paths.map { |path| read_profile_text(path) }.join("\n")
       end

atlas = Gemma4ProfileAtlas.new
...
unless atlas.groups.empty?
  ...
  atlas.groups.sort_by { |row| {-row.wait_ms, row.name} }.first(top_n).each do |row|
    ...
    printf "  %-32s %3d calls wait=%8.2f ms pct_group=%5.1f%% ratio_to_median=%4.2f\n",
...
unless atlas.conversions.empty?
  ...
  atlas.conversions.sort_by { |row| {-row.mib, row.name} }.first(top_n).each do |row|
    ...
    printf "  %-58s %3d calls %9.2f MiB %5.1f%% of conversions %5.1f%% total\n",
...
if atlas.cpu_fallback > 0
  puts "  Spike: cpu_fallback>0. Window=CPU matvec escape; corridor=Gemma layer/token span; potential=(fallback_calls,syncs,bytes). Legal move=cover missing Metal route or fail closed earlier."
end
...
if atlas.total_conversion_mib > 0 && atlas.total_logical_mib > 0
  ...
  if conversion_pct >= 15.0
    puts "  Ladder: conversion traffic is material (#{conversion_pct.round(1)}%). Window=F32/F16 staging; corridor=activation rows; potential=(conversion_mib,conversion_calls,syncs,wall). Legal move=fuse producer/consumer or keep staged type resident."
  end
end

if top = atlas.matmuls.sort_by { |row| {-row.mib, row.name} }.first?
  puts "  Ladder: dominant matmul scope '#{top.name}' carries #{top.mib.round(2)} MiB. Window=top logical-weight reader; corridor=layer/batch band; potential=(logical_weight_mib,calls,barriers). Legal move=prepack/fuse/re-route only if parity and paired wall improve."
end
...
unless atlas.groups.empty?
  ...
  group_median = median(waits)
  max_wait = waits.last
  if group_median > 0 && max_wait / group_median < 1.25
    puts "  Collapse: grouped waits are flat (max/median=#{(max_wait / group_median).round(2)}). This refutes a single bad group; reduce repeated per-layer work or dominant matmul bytes instead."
  else
    hot = atlas.groups.max_by(&.wait_ms)
    puts "  Diamond: hot group '#{hot.name}' is above median. Window=group conflict; corridor=command-buffer wave; potential=(group_wait_skew,syncs,bytes). Legal move=split/fuse only with ABBA wall proof."
  end
end

if atlas.traces.any?
  hot_trace = atlas.traces.max_by(&.ms)
  puts "  Spike/Ladder trace hint: '#{hot_trace.name}' consumed #{hot_trace.ms.round(2)} ms encode-side. Treat this as a construction hotspot only if wall/profile recompute agrees."
end

puts "  Dual frame: if a candidate does not lower the recomputed potential under paired timing, fall back to exact baseline/profile-only mode; do not stack speculative complexity."
