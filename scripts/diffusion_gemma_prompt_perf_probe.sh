...
probe_source="$repo_root/scripts/diffusion_gemma_sparse_loop_smoke.cr"
...
if [[ -n "$synthetic_canvas_lengths" ]]; then
  if [[ -z "$synthetic_prompt_lengths" ]]; then
    echo "SYNTHETIC_CANVAS_LENGTHS requires SYNTHETIC_PROMPT_LENGTHS" >&2
    exit 2
  fi
  read -r -a synthetic_lengths <<<"${synthetic_canvas_lengths//,/ }"
  if [[ "${#synthetic_lengths[@]}" -eq 0 ]]; then
    echo "SYNTHETIC_CANVAS_LENGTHS is empty" >&2
    exit 2
  fi
  for length in "${synthetic_lengths[@]}"; do
    if [[ ! "$length" =~ ^[1-9][0-9]*$ ]]; then
      echo "invalid SYNTHETIC_CANVAS_LENGTHS entry: $length" >&2
      exit 2
    fi
  done
  synthetic_canvas_lengths="$(IFS=,; echo "${synthetic_lengths[*]}")"
fi

if [[ ! -x "$bin" || "$probe_source" -nt "$bin" ]]; then
  if [[ -z "$bridge_o" ]]; then
  ...
  "$crystal_bin" build "$probe_source" \
    -o "$bin" \
    --link-flags="$bridge_o -framework Metal -framework Foundation -framework MetalPerformanceShaders -lc++"
fi

mkdir -p "$(dirname "$out")"
printf 'status\tcase\tprompt_bytes\tprompt_len\tcanvas_len\tcandidate_count\tload_ms\tprompt_route_ms\tprompt_projection_backend\tprompt_projection_fused_norm_rope\tprompt_cache_ms\tprompt_projection_ms\tprompt_projection_norm_ms\tprompt_projection_matmul_ms\tprompt_projection_assemble_ms\tprompt_projection_copy_ms\tprompt_projection_head_norm_ms\tprompt_projection_q_norm_ms\tprompt_projection_k_norm_ms\tprompt_projection_v_norm_ms\tprompt_projection_rope_ms\tprompt_projection_rope_table_ms\tprompt_projection_rope_apply_ms\tprompt_projection_rope_q_apply_ms\tprompt_projection_rope_k_apply_ms\tprompt_materialize_ms\tprompt_cache_ms_samples\tprompt_projection_ms_samples\tprompt_projection_norm_ms_samples\tprompt_projection_matmul_ms_samples\tprompt_projection_assemble_ms_samples\tprompt_projection_copy_ms_samples\tprompt_projection_head_norm_ms_samples\tprompt_projection_q_norm_ms_samples\tprompt_projection_k_norm_ms_samples\tprompt_projection_v_norm_ms_samples\tprompt_projection_rope_ms_samples\tprompt_projection_rope_table_ms_samples\tprompt_projection_rope_apply_ms_samples\tprompt_projection_rope_q_apply_ms_samples\tprompt_projection_rope_k_apply_ms_samples\tprompt_materialize_ms_samples\tprompt_cache_tokens_per_ms\tloop_ms_median\tloop_ms_samples\tloop_prediction_ms\tloop_decode_stack_ms\tloop_context_backend\tloop_context_batch_rows\tloop_context_fixed_gqa2\tloop_attention_residual_metal_rows\tloop_attention_residual_context_buffer\tloop_decode_qkv_ms\tloop_decode_context_ms\tloop_decode_context_score_ms\tloop_decode_context_softmax_ms\tloop_decode_context_value_ms\tloop_decode_attention_out_ms\tloop_decode_shared_ffn_ms\tloop_decode_moe_ffn_ms\tloop_decode_combine_scale_ms\tloop_output_head_ms\tloop_update_ms\tloop_regenerate_ms\tloop_proposal_ms\tloop_prediction_ms_samples\tloop_decode_stack_ms_samples\tloop_decode_qkv_ms_samples\tloop_decode_context_ms_samples\tloop_decode_context_score_ms_samples\tloop_decode_context_softmax_ms_samples\tloop_decode_context_value_ms_samples\tloop_decode_attention_out_ms_samples\tloop_decode_shared_ffn_ms_samples\tloop_decode_moe_ffn_ms_samples\tloop_decode_combine_scale_ms_samples\tloop_output_head_ms_samples\tloop_update_ms_samples\tloop_regenerate_ms_samples\tloop_proposal_ms_samples\tchosen\tprobs\tartifact\terr_artifact\n' >"$out"
...
run_case() {
  ...
  if [[ "$rc" -eq 0 ]]; then
    awk -F '\t' -v case="$name" -v bytes="$bytes" -v artifact="$tmp" -v err_artifact="$err" '
      ...
      NR == 2 {
        found = 1
        print "ok" "\t" case "\t" bytes "\t" $h["prompt_len"] "\t" $h["canvas_len"] "\t" $h["candidate_count"] "\t" $h["load_ms"] "\t" $h["prompt_route_ms"] "\t" $h["prompt_projection_backend"] "\t" $h["prompt_projection_fused_norm_rope"] "\t" $h["prompt_cache_ms"] "\t" $h["prompt_projection_ms"] "\t" $h["prompt_projection_norm_ms"] "\t" $h["prompt_projection_matmul_ms"] "\t" $h["prompt_projection_assemble_ms"] "\t" $h["prompt_projection_copy_ms"] "\t" $h["prompt_projection_head_norm_ms"] "\t" $h["prompt_projection_q_norm_ms"] "\t" $h["prompt_projection_k_norm_ms"] "\t" $h["prompt_projection_v_norm_ms"] "\t" $h["prompt_projection_rope_ms"] "\t" $h["prompt_projection_rope_table_ms"] "\t" $h["prompt_projection_rope_apply_ms"] "\t" $h["prompt_projection_rope_q_apply_ms"] "\t" $h["prompt_projection_rope_k_apply_ms"] "\t" $h["prompt_materialize_ms"] "\t" $h["prompt_cache_ms_samples"] "\t" $h["prompt_projection_ms_samples"] "\t" $h["prompt_projection_norm_ms_samples"] "\t" $h["prompt_projection_matmul_ms_samples"] "\t" $h["prompt_projection_assemble_ms_samples"] "\t" $h["prompt_projection_copy_ms_samples"] "\t" $h["prompt_projection_head_norm_ms_samples"] "\t" $h["prompt_projection_q_norm_ms_samples"] "\t" $h["prompt_projection_k_norm_ms_samples"] "\t" $h["prompt_projection_v_norm_ms_samples"] "\t" $h["prompt_projection_rope_ms_samples"] "\t" $h["prompt_projection_rope_table_ms_samples"] "\t" $h["prompt_projection_rope_apply_ms_samples"] "\t" $h["prompt_projection_rope_q_apply_ms_samples"] "\t" $h["prompt_projection_rope_k_apply_ms_samples"] "\t" $h["prompt_materialize_ms_samples"] "\t" $h["prompt_cache_tokens_per_ms"] "\t" $h["loop_ms_median"] "\t" $h["loop_ms_samples"] "\t" $h["loop_prediction_ms"] "\t" $h["loop_decode_stack_ms"] "\t" $h["loop_context_backend"] "\t" $h["loop_context_batch_rows"] "\t" $h["loop_context_fixed_gqa2"] "\t" col("loop_attention_residual_metal_rows") "\t" col("loop_attention_residual_context_buffer") "\t" $h["loop_decode_qkv_ms"] "\t" $h["loop_decode_context_ms"] "\t" $h["loop_decode_context_score_ms"] "\t" $h["loop_decode_context_softmax_ms"] "\t" $h["loop_decode_context_value_ms"] "\t" $h["loop_decode_attention_out_ms"] "\t" $h["loop_decode_shared_ffn_ms"] "\t" $h["loop_decode_moe_ffn_ms"] "\t" $h["loop_decode_combine_scale_ms"] "\t" $h["loop_output_head_ms"] "\t" $h["loop_update_ms"] "\t" $h["loop_regenerate_ms"] "\t" $h["loop_proposal_ms"] "\t" $h["loop_prediction_ms_samples"] "\t" $h["loop_decode_stack_ms_samples"] "\t" $h["loop_decode_qkv_ms_samples"] "\t" $h["loop_decode_context_ms_samples"] "\t" $h["loop_decode_context_score_ms_samples"] "\t" $h["loop_decode_context_softmax_ms_samples"] "\t" $h["loop_decode_context_value_ms_samples"] "\t" $h["loop_decode_attention_out_ms_samples"] "\t" $h["loop_decode_shared_ffn_ms_samples"] "\t" $h["loop_decode_moe_ffn_ms_samples"] "\t" $h["loop_decode_combine_scale_ms_samples"] "\t" $h["loop_output_head_ms_samples"] "\t" $h["loop_update_ms_samples"] "\t" $h["loop_regenerate_ms_samples"] "\t" $h["loop_proposal_ms_samples"] "\t" $h["last_chosen_texts"] "\t" $h["last_argmax_probabilities"] "\t" artifact "\t" err_artifact
      }
      END {
        if (!found) {
          exit 3
...
run_synthetic_lengths() {
  ...
  if [[ "$rc" -eq 0 ]]; then
    awk -F '\t' -v artifact="$tmp" -v err_artifact="$err" '
      ...
      NR >= 2 {
        found = 1
        case_name = "synthetic_p" $h["prompt_len"] "_c" $h["canvas_len"]
        print "ok" "\t" case_name "\t" $h["prompt_text_bytes"] "\t" $h["prompt_len"] "\t" $h["canvas_len"] "\t" $h["candidate_count"] "\t" $h["load_ms"] "\t" $h["prompt_route_ms"] "\t" $h["prompt_projection_backend"] "\t" $h["prompt_projection_fused_norm_rope"] "\t" $h["prompt_cache_ms"] "\t" $h["prompt_projection_ms"] "\t" $h["prompt_projection_norm_ms"] "\t" $h["prompt_projection_matmul_ms"] "\t" $h["prompt_projection_assemble_ms"] "\t" $h["prompt_projection_copy_ms"] "\t" $h["prompt_projection_head_norm_ms"] "\t" $h["prompt_projection_q_norm_ms"] "\t" $h["prompt_projection_k_norm_ms"] "\t" $h["prompt_projection_v_norm_ms"] "\t" $h["prompt_projection_rope_ms"] "\t" $h["prompt_projection_rope_table_ms"] "\t" $h["prompt_projection_rope_apply_ms"] "\t" $h["prompt_projection_rope_q_apply_ms"] "\t" $h["prompt_projection_rope_k_apply_ms"] "\t" $h["prompt_materialize_ms"] "\t" $h["prompt_cache_ms_samples"] "\t" $h["prompt_projection_ms_samples"] "\t" $h["prompt_projection_norm_ms_samples"] "\t" $h["prompt_projection_matmul_ms_samples"] "\t" $h["prompt_projection_assemble_ms_samples"] "\t" $h["prompt_projection_copy_ms_samples"] "\t" $h["prompt_projection_head_norm_ms_samples"] "\t" $h["prompt_projection_q_norm_ms_samples"] "\t" $h["prompt_projection_k_norm_ms_samples"] "\t" $h["prompt_projection_v_norm_ms_samples"] "\t" $h["prompt_projection_rope_ms_samples"] "\t" $h["prompt_projection_rope_table_ms_samples"] "\t" $h["prompt_projection_rope_apply_ms_samples"] "\t" $h["prompt_projection_rope_q_apply_ms_samples"] "\t" $h["prompt_projection_rope_k_apply_ms_samples"] "\t" $h["prompt_materialize_ms_samples"] "\t" $h["prompt_cache_tokens_per_ms"] "\t" $h["loop_ms_median"] "\t" $h["loop_ms_samples"] "\t" $h["loop_prediction_ms"] "\t" $h["loop_decode_stack_ms"] "\t" $h["loop_context_backend"] "\t" $h["loop_context_batch_rows"] "\t" $h["loop_context_fixed_gqa2"] "\t" col("loop_attention_residual_metal_rows") "\t" col("loop_attention_residual_context_buffer") "\t" $h["loop_decode_qkv_ms"] "\t" $h["loop_decode_context_ms"] "\t" $h["loop_decode_context_score_ms"] "\t" $h["loop_decode_context_softmax_ms"] "\t" $h["loop_decode_context_value_ms"] "\t" $h["loop_decode_attention_out_ms"] "\t" $h["loop_decode_shared_ffn_ms"] "\t" $h["loop_decode_moe_ffn_ms"] "\t" $h["loop_decode_combine_scale_ms"] "\t" $h["loop_output_head_ms"] "\t" $h["loop_update_ms"] "\t" $h["loop_regenerate_ms"] "\t" $h["loop_proposal_ms"] "\t" $h["loop_prediction_ms_samples"] "\t" $h["loop_decode_stack_ms_samples"] "\t" $h["loop_decode_qkv_ms_samples"] "\t" $h["loop_decode_context_ms_samples"] "\t" $h["loop_decode_context_score_ms_samples"] "\t" $h["loop_decode_context_softmax_ms_samples"] "\t" $h["loop_decode_context_value_ms_samples"] "\t" $h["loop_decode_attention_out_ms_samples"] "\t" $h["loop_decode_shared_ffn_ms_samples"] "\t" $h["loop_decode_moe_ffn_ms_samples"] "\t" $h["loop_decode_combine_scale_ms_samples"] "\t" $h["loop_output_head_ms_samples"] "\t" $h["loop_update_ms_samples"] "\t" $h["loop_regenerate_ms_samples"] "\t" $h["loop_proposal_ms_samples"] "\t" $h["last_chosen_texts"] "\t" $h["last_argmax_probabilities"] "\t" artifact "\t" err_artifact
      }
      END {
        if (!found) {
          exit 3
