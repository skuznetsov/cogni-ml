...
mkdir -p "$(dirname "$out")"
printf 'status\tcase\tprompt_bytes\tprompt_len\tcanvas_len\tcandidate_count\tload_ms\tprompt_route_ms\tprompt_projection_backend\tprompt_cache_ms\tprompt_cache_tokens_per_ms\tloop_ms_median\tloop_ms_samples\tchosen\tprobs\tartifact\terr_artifact\n' >"$out"
...
run_case() {
  ...
  if [[ "$rc" -eq 0 ]]; then
    awk -F '\t' -v case="$name" -v bytes="$bytes" -v artifact="$tmp" -v err_artifact="$err" '
      ...
      NR == 2 {
        found = 1
        print "ok" "\t" case "\t" bytes "\t" $h["prompt_len"] "\t" $h["canvas_len"] "\t" $h["candidate_count"] "\t" $h["load_ms"] "\t" $h["prompt_route_ms"] "\t" $h["prompt_projection_backend"] "\t" $h["prompt_cache_ms"] "\t" $h["prompt_cache_tokens_per_ms"] "\t" $h["loop_ms_median"] "\t" $h["loop_ms_samples"] "\t" $h["last_chosen_texts"] "\t" $h["last_argmax_probabilities"] "\t" artifact "\t" err_artifact
      }
      END {
        if (!found) {
          exit 3
