...
if [[ "$full_replay" == "1" ]]; then
  RESIDENT_BUDGET_GATE="$resident_budget_gate" scripts/rpi5_q6_capture_smoke.sh "$prompt"
  resident_budget_gate_ran="$resident_budget_gate"
else
  SKIP_REPLAY=1 scripts/rpi5_q6_capture_smoke.sh "$prompt"
fi

printf "allowed_head_regression_result\tmarker=ok\tcapture=ok\tfull_replay=%s\tresident_budget_gate=%s\n" "$full_replay" "$resident_budget_gate_ran"
