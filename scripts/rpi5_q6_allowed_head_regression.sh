...
the resident budget, resident stream, and resident stdin gates.
...
Environment:
  ...
  RESIDENT_STDIN_GATE=1
                    With FULL_REPLAY=1, run resident binary-stdin protocol gate.
USAGE
  exit 2
}
...
resident_stdin_gate="${RESIDENT_STDIN_GATE:-1}"
resident_stdin_gate_ran=0
...
if [[ "$full_replay" == "1" ]]; then
  ...
  RESIDENT_STDIN_GATE="$resident_stdin_gate" \
  ...
  resident_stdin_gate_ran="$resident_stdin_gate"
else
  SKIP_REPLAY=1 scripts/rpi5_q6_capture_smoke.sh "$prompt"
fi

printf "allowed_head_regression_result\tmarker=ok\tcapture=ok\tfull_replay=%s\tresident_budget_gate=%s\tresident_stream_gate=%s\tresident_stdin_gate=%s\n" "$full_replay" "$resident_budget_gate_ran" "$resident_stream_gate_ran" "$resident_stdin_gate_ran"
